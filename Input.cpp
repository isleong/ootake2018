...
・６ボタン＆連射専用ボタンと２プレイヤーパッドのプレイレコードにも対応した。
  v2.15
・XInputの状態を参照する際に、つながっていないナンバーのXInputを読もうとすると
  処理がめちゃめちゃ重いので、XInput用パッドがつながっている本数ぶんだけ状態を
  参照するようにした。v2.89
...
typedef struct
{
	DIJOYSTATE2					joyState; //ボタンの状態。XInput利用時には非使用。v2.88記
	Uint32						buttonState; //Kitao更新。16ボタンに対応。ハットスイッチぶんも入れたのでUint32に。※XInput利用時は10ボタンまで。v2.88記
} JOYSTICK; //Kitao更新


static char						_DIKeys[256]; // キーボードのステータスを代入する配列

static Sint32					_nJoySticks = 0; //DirectInput接続のジョイパッドの数
static Sint32					_nJoySticksX = 0; //XInput接続のジョイパッドの数。v2.89
...
static BOOL						_bSorcerianOpSkip; //v2.82追加
static BOOL						_bDarius26Tai; //v2.85追加
static BOOL						_bRTypeCD; //v2.90追加
static Sint32					_RTypeCDCount; //v2.90追加

static JOYSTICK					_Joystick[N_MAXJOYSTICK]; //DirectInput用
static JOYSTICK					_JoystickX[4]; //Xinput用。v2.88追加
	...
	LPVOID				pvRef)
{
	...
	pDIDJoy->QueryInterface(IID_IDirectInputDevice2, (LPVOID*)&_pDIDJoy[_nJoySticks]);
	pDIDJoy->Release();
...
INPUT_Init()
{
	...
	//DirectInput対応ジョイパッドを列挙する
	_pDI->EnumDevices(DIDEVTYPE_JOYSTICK, DIEnumDevicesCallback, NULL, DIEDFL_ATTACHEDONLY);

	//最初にDirectInput対応ジョイパッドのアクセス権を得ておく。Kitao追加
	for (i = 0; i < _nJoySticks; i++)
		_pDIDJoy[i]->Acquire();

	//XInput対応ジョイパッドがつながっている本数をチェック。つながっていないパッドを参照しようとするとめちゃめちゃ重いため、事前にチェックする必要がある。v2.89
	for (i = 0; i < 4; i++) //XInputは最大４つまで。v2.89
	{
		ZeroMemory(&state, sizeof(XINPUT_STATE));
		if (XInputGetState(i, &state) == ERROR_SUCCESS) //ジョイパッドがつながっていた
			_nJoySticksX++;
	}
	...
	Sint32	a)
{
	...
	else if (a <= 799) //DirectX対応ジョイパッドのキー・ボタンが設定されていた場合
	{
		a -= 300;
		winJoyID = 0;
		while (a >= 100)
		...
		if (a < 50) //十字キー・ハットスイッチ・Ｚ軸トリガなら
		{
			switch (a)
			{
				...
				case 8: //8～9 Z軸トリガ
					//PRINTF("%d, %d, %d  lZ! test",a,_Joystick[winJoyID].buttonState,((_Joystick[winJoyID].buttonState & INPUT_JOYSTICK_L2ZTRIGGER) != 0)); //Test用
					return (_Joystick[winJoyID].buttonState & INPUT_JOYSTICK_L2ZTRIGGER) != 0;
				case 9:
					return (_Joystick[winJoyID].buttonState & INPUT_JOYSTICK_R2ZTRIGGER) != 0;
			}
		}
		else //ボタン類なら
		{
	...
	else //(A>=800) XInput対応ジョイパッドのキー・ボタンが設定されていた場合
	{
		a -= 800;
		winJoyID = 0;
		while (a >= 100)
		...
		if (a < 50) //十字キー・ハットスイッチ・Ｚ軸トリガなら
		{
			...
			{
				case 0: //0～3 十字キー
					return (_JoystickX[winJoyID].buttonState & INPUT_JOYSTICK_UP) != 0;
				case 1:
					return (_JoystickX[winJoyID].buttonState & INPUT_JOYSTICK_RIGHT) != 0;
				case 2:
					return (_JoystickX[winJoyID].buttonState & INPUT_JOYSTICK_DOWN) != 0;
				case 3:
					return (_JoystickX[winJoyID].buttonState & INPUT_JOYSTICK_LEFT) != 0;
			}
		}
		else //ボタン類なら
		{
			...
			{
				case 0:
					return (_JoystickX[winJoyID].buttonState & INPUT_JOYSTICK_BUTTON1) != 0;
				case 1:
					return (_JoystickX[winJoyID].buttonState & INPUT_JOYSTICK_BUTTON2) != 0;
				case 2:
					return (_JoystickX[winJoyID].buttonState & INPUT_JOYSTICK_BUTTON3) != 0;
				case 3:
					return (_JoystickX[winJoyID].buttonState & INPUT_JOYSTICK_BUTTON4) != 0;
				case 4:
					return (_JoystickX[winJoyID].buttonState & INPUT_JOYSTICK_BUTTON5) != 0;
				case 5:
					return (_JoystickX[winJoyID].buttonState & INPUT_JOYSTICK_BUTTON6) != 0;
				case 6:
					return (_JoystickX[winJoyID].buttonState & INPUT_JOYSTICK_BUTTON7) != 0;
				case 7:
					return (_JoystickX[winJoyID].buttonState & INPUT_JOYSTICK_BUTTON8) != 0;
				case 8:
					return (_JoystickX[winJoyID].buttonState & INPUT_JOYSTICK_BUTTON9) != 0;
				case 9:
					return (_JoystickX[winJoyID].buttonState & INPUT_JOYSTICK_BUTTON10) != 0;
			}
		}
	}
...
//コントローラ１のRUNボタンの状態を返す。CDROM.cppから利用。v2.90追加
BOOL
INPUT_CheckRunButtonState()
{
	return INPUT_CheckButtonState(_PcePad[1].buttonRun);
}
	...
	int	a)
{
		...
		if (a < 50) //十字キー・ハットスイッチ・Ｚ軸トリガなら
		{
			switch (a)
			{
				...
				case 7:
					...
					return;
				case 8: //8～9 Z軸トリガ
					_Joystick[winJoyID].buttonState |= INPUT_JOYSTICK_L2ZTRIGGER;
					return;
				case 9:
					_Joystick[winJoyID].buttonState |= INPUT_JOYSTICK_R2ZTRIGGER;
					return;
			}
		}
		else //ボタン類なら
	...
	else //(A>=800) XInput対応ジョイパッドのキー・ボタンが設定されていた場合
	{
		...
		if (a < 50) //十字キー・ハットスイッチ・Ｚ軸トリガなら
		{
			...
			{
				case 0: //0～3 十字キー
					_JoystickX[winJoyID].buttonState |= INPUT_JOYSTICK_UP;
					return;
				case 1:
					_JoystickX[winJoyID].buttonState |= INPUT_JOYSTICK_RIGHT;
					return;
				case 2:
					_JoystickX[winJoyID].buttonState |= INPUT_JOYSTICK_DOWN;
					return;
				case 3:
					_JoystickX[winJoyID].buttonState |= INPUT_JOYSTICK_LEFT;
					return;
			}
		}
		else //ボタン類なら
		{
			...
			{
				case 0:
					_JoystickX[winJoyID].buttonState |= INPUT_JOYSTICK_BUTTON1;
					return;
				case 1:
					_JoystickX[winJoyID].buttonState |= INPUT_JOYSTICK_BUTTON2;
					return;
				case 2:
					_JoystickX[winJoyID].buttonState |= INPUT_JOYSTICK_BUTTON3;
					return;
				case 3:
					_JoystickX[winJoyID].buttonState |= INPUT_JOYSTICK_BUTTON4;
					return;
				case 4:
					_JoystickX[winJoyID].buttonState |= INPUT_JOYSTICK_BUTTON5;
					return;
				case 5:
					_JoystickX[winJoyID].buttonState |= INPUT_JOYSTICK_BUTTON6;
					return;
				case 6:
					_JoystickX[winJoyID].buttonState |= INPUT_JOYSTICK_BUTTON7;
					return;
				case 7:
					_JoystickX[winJoyID].buttonState |= INPUT_JOYSTICK_BUTTON8;
					return;
				case 8:
					_JoystickX[winJoyID].buttonState |= INPUT_JOYSTICK_BUTTON9;
					return;
				case 9:
					_JoystickX[winJoyID].buttonState |= INPUT_JOYSTICK_BUTTON10;
					return;
			}
		}
	}
	...
	BOOL	bRecord) //Kitao追加。記録・再生を行うならTRUE。
{
	...
	if ((bRecord)&&(_bPlayRecord))
	{
		...
		for (i = 0; i < 4; ++i)
			_JoystickX[i].buttonState = 0; //XInputジョイスティックのステータスをクリア
		for (i = 0; i < _nJoySticks; ++i)
			_Joystick[i].buttonState = 0; //DirectInputジョイスティックのステータスをクリア
	...
	else //通常時
	{
		...
		// XInput対応ジョイスティックが存在する時は、そのジョイスティックの状態を読む。
		for (i=0; i<_nJoySticksX; i++)
		{
			ZeroMemory(&state, sizeof(XINPUT_STATE));
			dwResult = XInputGetState(i, &state);
			if (dwResult == ERROR_SUCCESS) //ジョイパッドがつながっていた
			{
				// ボタンの状態を更新する (とりあえず 12 個のボタンに対応) Kitao更新。16個のボタンに対応
				_JoystickX[i].buttonState = 0;
				if(state.Gamepad.wButtons & XINPUT_GAMEPAD_A)				_JoystickX[i].buttonState |= INPUT_JOYSTICK_BUTTON1;
				if(state.Gamepad.wButtons & XINPUT_GAMEPAD_B)				_JoystickX[i].buttonState |= INPUT_JOYSTICK_BUTTON2;
				if(state.Gamepad.wButtons & XINPUT_GAMEPAD_X)				_JoystickX[i].buttonState |= INPUT_JOYSTICK_BUTTON3;
				if(state.Gamepad.wButtons & XINPUT_GAMEPAD_Y)				_JoystickX[i].buttonState |= INPUT_JOYSTICK_BUTTON4;
				if(state.Gamepad.wButtons & XINPUT_GAMEPAD_START)			_JoystickX[i].buttonState |= INPUT_JOYSTICK_BUTTON5;
				if(state.Gamepad.wButtons & XINPUT_GAMEPAD_BACK)			_JoystickX[i].buttonState |= INPUT_JOYSTICK_BUTTON6;
				if(state.Gamepad.wButtons & XINPUT_GAMEPAD_LEFT_THUMB)		_JoystickX[i].buttonState |= INPUT_JOYSTICK_BUTTON7;//左アナログスティック押し込み
				if(state.Gamepad.wButtons & XINPUT_GAMEPAD_RIGHT_THUMB)		_JoystickX[i].buttonState |= INPUT_JOYSTICK_BUTTON8;//右アナログスティック押し込み
				if(state.Gamepad.wButtons & XINPUT_GAMEPAD_LEFT_SHOULDER)	_JoystickX[i].buttonState |= INPUT_JOYSTICK_BUTTON9;
				if(state.Gamepad.wButtons & XINPUT_GAMEPAD_RIGHT_SHOULDER)	_JoystickX[i].buttonState |= INPUT_JOYSTICK_BUTTON10;
				if(state.Gamepad.wButtons & XINPUT_GAMEPAD_DPAD_UP)			_JoystickX[i].buttonState |= INPUT_JOYSTICK_UP;
				if(state.Gamepad.wButtons & XINPUT_GAMEPAD_DPAD_RIGHT)		_JoystickX[i].buttonState |= INPUT_JOYSTICK_RIGHT;
				if(state.Gamepad.wButtons & XINPUT_GAMEPAD_DPAD_DOWN)		_JoystickX[i].buttonState |= INPUT_JOYSTICK_DOWN;
				if(state.Gamepad.wButtons & XINPUT_GAMEPAD_DPAD_LEFT)		_JoystickX[i].buttonState |= INPUT_JOYSTICK_LEFT;
			}
		}

		// DirectInput対応ジョイスティックが存在する時は、そのジョイスティックの状態も読む。
		for (i=0; i<_nJoySticks; i++)
		{
			...
			//Kitao更新。Ｚ軸トリガ（アナログ対応コントローラのL2,R2ボタンでよく使われる）にも対応。v2.81
			if (_Joystick[i].joyState.lZ > +25)	_Joystick[i].buttonState |= INPUT_JOYSTICK_L2ZTRIGGER;
			if (_Joystick[i].joyState.lZ < -25)	_Joystick[i].buttonState |= INPUT_JOYSTICK_R2ZTRIGGER;
			//PRINTF("%d, %d, %d, %d  lZ! test",_Joystick[i].buttonState, _Joystick[i].joyState.lZ, _PcePad[1].buttonL, _PcePad[1].buttonR);
		}
	...
	if (_bGameSetting) //速度を落とさないためこのフラグで、処理を行うかどうかを先に判定する。
	{
		...
		//ソーサリアンでオープニングロゴをスキップする。v2.82
		if (_bSorcerianOpSkip)
		{
			set_button_state(_PcePad[1].button1); //１ボタンが押しっぱなしにされた状態にする
		}
		//スーパーダライアスで26体戦えますかの裏技をコントローラ１だけの入力で実現する。v2.85
		if (_bDarius26Tai)
		{
			//コントローラ２の上＋I＋RUNボタンが押しっぱなしにされた状態にする。
			if (_PcePad[2].buttonU == -1) //コントローラ２の上キーの設定がリセットされている場合
				_PcePad[2].buttonU = 902; //仮想的に設定(902は実際の入力機器では入力できない値)しておく
			set_button_state(_PcePad[2].buttonU);
			if (_PcePad[2].button1 == -1) //コントローラ２のIボタンの設定がリセットされている場合
				_PcePad[2].button1 = 902; //仮想的に設定(902は実際の入力機器では入力できない値)しておく
			set_button_state(_PcePad[2].button1);
			if (_PcePad[2].buttonRun == -1) //コントローラ２のRUNボタンの設定がリセットされている場合
				_PcePad[2].buttonRun = 902; //仮想的に設定(902は実際の入力機器では入力できない値)しておく
			set_button_state(_PcePad[2].buttonRun);
			//コントローラ１の下キーが押されている場合は、SELCTボタンも押しっぱなしにされた状態にする。
			if (INPUT_CheckButtonState(_PcePad[1].buttonD))
				set_button_state(_PcePad[1].buttonSel); //SELECTボタンが押しっぱなしにされた状態にする
		}
		//R-TYPE Complete CDでデモをスキップする機能。v2.90
		if (_bRTypeCD)
		{
			if (_RTypeCDCount > 0)
			{
				_RTypeCDCount--;
				reset_button_state(_PcePad[1].buttonRun); //RUNボタンを離す
			}
			else
				set_button_state(_PcePad[1].buttonRun); //RUNボタンが押しっぱなしにされた状態にする
		}
	}
}
...
//Kitao追加。各ゲームごとの自動操作設定をしているかどうかを返す。v2.85
BOOL
INPUT_GetGameSetting()
{
	return _bGameSetting;
}

//Kitao追加。各ゲームごとの自動操作設定をしているかどうかを設定する。v2.89
void
INPUT_SetGameSetting(
	BOOL	bGameSetting)
{
	_bGameSetting = bGameSetting;
}
...
SetGameSetting()
{
	return _bKonamiStereo || _bGradiusII || _bSorcerianOpSkip || _bDarius26Tai || _bRTypeCD;
}
...
//Kitao追加。ソーサリアンのオープニングロゴを飛ばしてすぐ星の画面へ。v2.82
void
INPUT_SetSorcerianOpSkip(
...
//Kitao追加。スーパーダライアスで26体戦えますかの裏技を使う際コントローラ２の操作を省けるように。v2.85
void
INPUT_SetDarius26Tai(
	BOOL	bDarius26Tai)
{
	_bDarius26Tai = bDarius26Tai;
	_bGameSetting = SetGameSetting();
}

//Kitao追加。R-TYPE Complete CDでデモをスキップする機能。v2.90
void
INPUT_SetRTypeCD(
	BOOL	bRTypeCD)
{
	reset_button_state(_PcePad[1].buttonRun); //RUNボタンを離す
	if (bRTypeCD)
		_RTypeCDCount = 9;//9。「RUNを押す直前」に「RUNボタンを離し続ける時間」をセット。２面クリア時、非FastCD時にRUN押したままで8だと少なすぎてRUNの反応が無い場合があった。
	_bRTypeCD = bRTypeCD;
	_bGameSetting = SetGameSetting();
}
BOOL
