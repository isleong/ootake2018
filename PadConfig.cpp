...
・ステートセーブ用ボタンの設定を追加。
・ROMファイルを開くボタンの設定を追加。
・CDインストールされたゲーム（CUE）を開くボタンの設定を追加。
・スクリーンショット用ボタンの設定を追加。
・Z軸トリガ（PS4用DS4をInputMapper(XInputへ変換)入れて利用した場合などで存在）
  にも対応した。v2.81
・XInput（Xbox360規格のジョイパッド）への対応。公開用のOotakeの場合、
　古い"dinput.lib"を使ってビルドしているため、DirectInputのほうが低遅延にな
　る。マイクロソフトのドライバが入っていればXInput用ジョイパッドもDirectInput
　用として利用（自動で両対応）できる。v2.88
...
//Kitao更新。ハットスイッチ，16ボタン，Ｚ軸トリガにも対応。
static const Sint32	button[INPUT_NUM_BUTTON] =
{
	...
	INPUT_JOYSTICK_L2ZTRIGGER, INPUT_JOYSTICK_R2ZTRIGGER
};
...
static Sint32		_nJoySticks; //DirectInput接続のジョイパッドの数
static Sint32		_nJoySticksX; //XInput接続のジョイパッドの数。v2.89
...
static Sint32		_Mode; //Kitao追加。0…通常の設定。1…連射専用ボタンの設定。2…早回し用ボタンの設定。3…ステートセーブ用ボタンの設定。4…ステートロード用ボタンの設定。
						   ...
						   //			8…ポーズ用ボタンの設定。9…スプライトとBGの非表示・表示切替ボタンの設定。10…ROMイメージを開くボタンの設定。11…CDインストールのゲーム（CUE）を開くボタンの設定。
...
//DirectInput対応ジョイスティックの入力状況を更新する。
static void
joypad_update_state()
{
	int		i;
	...
	for (i = 0; i < _nJoySticks; i++)
	{
		...
		// ボタンの状態を更新する 。Kitao更新。16個のボタン，ハットスイッチ，Ｚ軸トリガに対応。v2.81
		...
		if (_Joystick[i].joyState.lY < -250)	_Joystick[i].buttonState |= INPUT_JOYSTICK_UP; //設定時は安易にスティックが傾いて誤動作してしまわないように-250まで遊びを許す。
		...
		if (_Joystick[i].joyState.lZ > +50) _Joystick[i].buttonState |= INPUT_JOYSTICK_L2ZTRIGGER; //設定時は安易にスティックが傾いて誤動作してしまわないように+50まで遊びを許す。
		if (_Joystick[i].joyState.lZ < -50) _Joystick[i].buttonState |= INPUT_JOYSTICK_R2ZTRIGGER;
	}
}

//Kitao追加。XInput対応ジョイスティックの入力状況を更新する。v2.88
static void
joypadx_update_state()
{
	int				i;
	DWORD			dwResult;
	XINPUT_STATE	state;
	
	//InputX対応ジョイスティックの入力状況を更新
	...
	Sint16*		pButton)
{
	...
	while (theButton < 0)
	{
		...
		joypadx_update_state(); //XInput対応ジョイスティックの状態を読む。v2.88追加。DInputと比べて反応が早ければこちらが優先される。古いdinput.lib(October 2004)でビルドした場合は、DInputのほうが反応が早い場合もある。
		joypad_update_state(); //DInput対応ジョイスティックの状態を読む。

		//XInput対応のジョイスティック。v2.88追加。基本的にこちらのほうが反応が早いのでDirectInputよりも先に参照する。v2.89
		for (q=0; q<=_nJoySticksX-1; q++) //最大４本のジョイスティック
			for (i=0; i<=INPUT_NUM_BUTTON-1; i++)
				if (_JoystickX[q].buttonState & button[i]) //ボタンが押されていたら
				{
					//押されたボタンが離されるまで待つ
					while (_JoystickX[q].buttonState & button[i])
					{
						Sleep(1);
						if (!pump_message())
							return FALSE;
					...
					theButton = i;
					if (theButton <= 7) //十字キー
						*pButton = (700 + theJoystick*100) + theButton;
					else //ボタン1～10
						*pButton = (700 + theJoystick*100 +50) + (theButton -8);
					
					return TRUE;
				}

		//DirectInput対応のジョイスティック
		for (q=0; q<=_nJoySticks-1; q++) //Kitao追加。つながれている全てのジョイスティックのボタンを受け付けるようにした。
			for (i=0; i<=INPUT_NUM_BUTTON-1; i++)
				if (_Joystick[q].buttonState & button[i]) //ボタンが押されていたら
				{
					...
					else if (theButton <= 23) //ボタン1～16
						*pButton = (200 + theJoystick*100 +50) + (theButton -8);
					else //Ｚ軸トリガ
						*pButton = (200 + theJoystick*100) + (theButton -16);
		...
		for (i=0; i<=255; i++)
			if ((_DIKeys[i] & 0x80)&&(i != DIK_ESCAPE)&&(i != DIK_CAPITAL)&&(i != DIK_KANJI)&& //何かキーが押されていたら（Esc＆Caps＆漢字(半角/全角)キー以外）。v0.83更新(Kiさん修正ありがとうございます)
				(i != DIK_KANA)) //「かなキー」も無効に。(押してしまうとキーを受け付けなくなる)。v1.33追加
			{
				//押されたボタンが離されるまで待つ
	...
	switch (mode)
	{
		...
		case 1:	//通常の設定or連射専用ボタンの設定
			...
			{	//通常時
				...
				{
					if (JOYPAD_GetChange3to12()) //v2.87追加
					{
						add_text("Press a button for \"button IIIr[=I+II]\"..."); if (!get_button(&buttonIII)) return FALSE;
						add_text("Press a button for \"button IIIs[=RUN]\"...");  if (!get_button(&buttonIV)) return FALSE;
					}
					else
					{
		...
		case 8: //ポーズ用ボタンの設定
			...
			break;
		case 9: //スプライトとBGの非表示・表示切替用ボタンの設定
		...
		case 10: //ROMファイルを開くボタンの設定
			add_text("Setting of \"Rom image Open Button\": (\"Esc\"=Abort \"F1\"=Clear)");
			add_text("Press a button for \"Rom image Open Button\"...");   if (!get_button(&buttonI)) return FALSE;
			break;
		case 11: //CDインストールされたゲーム（CUE）を開くボタンの設定
			add_text("Setting of \"Play FullInst CD-Game Button\": (\"Esc\"=Abort \"F1\"=Clear)");
			add_text("Press a button for \"Play FullInstalled CD-Game Button\"...");   if (!get_button(&buttonI)) return FALSE;
			break;
	}
	...
	LPARAM		lParam)
{
	...
	{
	case WM_CREATE:
		EnableWindow(_hPWnd, FALSE);//Kitao追加。親ウインドウを無効化してモーダルに。
...
padconfig_main()
{
	...
	_nJoySticksX = 0;
	ZeroMemory(_JoystickX, sizeof(_JoystickX));
...
PADCONFIG_Init(
	...
	Sint32	 	mode, //Kitao追加。0…通常の設定。1…連射専用ボタンの設定。2…早回し用ボタンの設定。3…ステートセーブ用ボタンの設定。4…ステートロード用ボタンの設定。5…スクリーンショット用ボタンの設定。
					  //		   6…ファンクション用ボタンの設定。7…ファンクションボタン時のステートセーブ＆ロード用ボタンの設定。8…ポーズ用ボタンの設定。9…スプライトとBGの非表示・表示切替ボタンの設定。
