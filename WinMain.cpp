...
//Kitao追加。ウィンドウ移動完了時＆アクティブ時にポーズを解除する処理
static void
cancelPause()
{
	RECT	rc;

	if (MAINBOARD_GetPause() == FALSE) //ポーズ中ではなかった場合
		return;

	if (APP_GetPauseNoRelease()) //フルスクリーンとウィンドウのモード切替中や、ウィンドウ最小化中はポーズを解除しない。（音の乱れや誤動作を防ぐのに必要）
		return;
	...
	{
		//もし、ウィンドウ右側と下側が画面からはみ出ていた場合は、ポーズされたままにする。
			...
			(rc.bottom <= GetSystemMetrics(SM_CYSCREEN))) //右側と下側がはみ出していなければ
		{
			...
			if (rc.top < 0) //上側がはみ出していた場合
				MoveWindow(_hMainWnd, rc.left, 0,
							rc.right-rc.left, rc.bottom-rc.top, TRUE);//ウィンドウの位置を変える
			MAINBOARD_Pause(FALSE);//ポーズ解除
		}
	}
}
...
wnd_proc(
	...
	LPARAM		lParam)
{
	short		delta;//Kitao追加。マウスホイール検出用
	Sint32		a;
	HWND		hP;
	MSG			msg;
	BOOL		bOk;
	...
	switch(uMsg)
	{
	//Kitao追加。DirectInputのアクセス権の取得をここでするようにした。（高速化）
	case WM_ACTIVATE:
		if (APP_GetInit()) //初期化が完了しているときのみ。v2.77
		{
			if (wParam == 0) //非アクティブになるとき
			{
				if ((APP_GetDrawMethod() == 2)&&(APP_GetFullScreen())) //DirectDrawのフルスクリーンで非アクティブになる場合
				{
					hP = GetWindow((HWND)lParam, GW_OWNER); //ダイアログの親ハンドル
					if (hP != _hMainWnd) //Ootakeのダイアログがアクティブになる際には処理しない。
					{
						APP_ToggleFullscreen();//Windowモードへ強制的に戻す。こうしない再びフォーカスを得られない。
						APP_RunEmulator(FALSE);//画面を１クリックでポーズ解除できるようにメニューを表示してのポーズ。
					}
				}
				if (APP_GetInactivePause())
					if (!MAINBOARD_GetPause()) //ポーズしていない場合
						MAINBOARD_Pause(TRUE);//ポーズ
			}
			else //アクティブになるとき
			{
				INPUT_Acquire(); //DirectInputのアクセス権利を得る
				if (IsIconic(hWnd) == FALSE) //最小化されているときはここでポーズを解除しない（最小化直後に他のアプリフォーカスが移るときに、一瞬Ootakeがアクティブになるため）
					cancelPause(); //ポーズ解除処理。最小化からノーマルに戻されたときに必要。
			}
		}
		break;

	//Kitao追加。ウィンドウを移動ドラッグ中はエミュレータ動作をポーズする。
	case WM_ENTERSIZEMOVE:
		if (!MAINBOARD_GetPause()) //ポーズしていない場合
			MAINBOARD_Pause(TRUE);//ポーズ
		break;
	//Kitao追加。ウィンドウの移動ドラッグが終わったらエミュレータ動作のポーズを解除する。
	case WM_EXITSIZEMOVE:
		if (!APP_GetFullScreen()) //フルスクリーン時は機能させない(他のアプリへフォーカスが移ったときに誤動作するため)
		{
			APP_SaveWindowPosition(); //ウィンドウの位置を保存しておく。
			cancelPause(); //ポーズ解除処理
		}
		break;
	...
	case WM_KEYDOWN:
		switch (wParam)
		{
		case VK_F1: //Kitao更新 CDゲームプレイ中は、[F1]キーでCDの入れ替えもできるようにした。
		...
		case VK_MENU: //[Alt]キー v2.66更新。Altキーを離してからMENU表示するようにした。
				APPEVENT_Set(APPEVENT_SHOWMENU, NULL); //MENU表示
				break;
		...
		case VK_F12:
			...
			break;
		case 'R': //Kitao追加。[R]キー
			APP_RunEmulator(FALSE); //ポーズしてメニューを出す。フルスクリーン時に必要。v2.70
	...
	case WM_SYSKEYDOWN:
		switch (wParam)
		{
			case VK_RETURN: //[Enter]キー v2.66追加
					if (GetKeyState(VK_MENU) < 0) //Altキーを押しながらの場合
					{
						APPEVENT_Set(APPEVENT_SCREEN_FULLSCREEN, NULL); //フルスクリーン切替
						while (PeekMessage(&msg, hWnd, WM_KEYFIRST, WM_KEYLAST, PM_REMOVE)); //キーバッファをクリア
					}
					break;
			case '1': //[1]キー v2.66追加
					if (!APP_GetFullScreen()) //ウィンドウモードの場合のみ有効
						if (GetKeyState(VK_MENU) < 0) //Altキーを押しながらの場合
							{
								APPEVENT_Set(APPEVENT_SCREEN_X1, NULL); //ウィンドウサイズをx1に。
								while (PeekMessage(&msg, hWnd, WM_KEYFIRST, WM_KEYLAST, PM_REMOVE)); //キーバッファをクリア
							}
					break;
			case '2': //[2]キー v2.66追加
					if (!APP_GetFullScreen()) //ウィンドウモードの場合のみ有効
						if (GetKeyState(VK_MENU) < 0) //Altキーを押しながらの場合
							{
								APPEVENT_Set(APPEVENT_SCREEN_X2, NULL); //ウィンドウサイズをx2に。
								while (PeekMessage(&msg, hWnd, WM_KEYFIRST, WM_KEYLAST, PM_REMOVE)); //キーバッファをクリア
							}
					break;
			case '3': //[3]キー v2.66追加
					if (!APP_GetFullScreen()) //ウィンドウモードの場合のみ有効
						if (GetKeyState(VK_MENU) < 0) //Altキーを押しながらの場合
							{
								APPEVENT_Set(APPEVENT_SCREEN_X3, NULL); //ウィンドウサイズをx3に。
								while (PeekMessage(&msg, hWnd, WM_KEYFIRST, WM_KEYLAST, PM_REMOVE)); //キーバッファをクリア
							}
					break;
			case '4': //[4]キー v2.66追加
					if (!APP_GetFullScreen()) //ウィンドウモードの場合のみ有効
						if (GetKeyState(VK_MENU) < 0) //Altキーを押しながらの場合
							{
								APPEVENT_Set(APPEVENT_SCREEN_X4, NULL); //ウィンドウサイズをx4に。
								while (PeekMessage(&msg, hWnd, WM_KEYFIRST, WM_KEYLAST, PM_REMOVE)); //キーバッファをクリア
							}
					break;
			case VK_MENU: //上記以外で[Alt]キーを押した場合
					return 1; //Altキーを押した際のメニュー表示処理をカット。v2.87
		}
		break;

	case WM_LBUTTONDOWN:
		if ((!MOUSE_IsConnected())||(MAINBOARD_GetPause()))
				APPEVENT_Set(APPEVENT_RUN, NULL);//Kitao追加。マウス非使用かエミュレータがポーズ中なら、クリックでポーズor解除。
	...
	case WM_RBUTTONDOWN:
		if ((!MOUSE_IsConnected())||(MAINBOARD_GetPause()))
				APPEVENT_Set(APPEVENT_RUN, NULL);//Kitao追加。マウス非使用かエミュレータがポーズ中なら、右クリックでポーズor解除。
	...
	case WM_MBUTTONDOWN: //Kitao追加。マウスの真ん中ボタンがダウンされたとき
		if (!MOUSE_IsConnected())
			APPEVENT_Set(APPEVENT_RUN, NULL);//真ん中ボタンクリックでポーズor解除。
	...
	//Kitao追加。CD-ROMの挿入時、取り出し時に自動で対応する。v2.84
	case WM_DEVICECHANGE:
		if (APP_GetCDGame() && !APP_GetCueFile()) //CDドライブでプレイしている場合
		{
			PDEV_BROADCAST_HDR	lpdb = (PDEV_BROADCAST_HDR)lParam;
			char				drive[80];

			//ドライブレターが現在CDゲームをプレイしているドライブと一致しているかを確認する
			bOk = FALSE; //一致したらTRUE
			if (lpdb->dbch_devicetype == DBT_DEVTYP_VOLUME)
			{
				...
				{
					wsprintf(drive, "%c", FirstDriveFromMask(lpdbv->dbcv_unitmask));
					if (drive[0] == CDIF_GetDriveLetters(CDIF_GetDeviceInUse()))
						bOk = TRUE;
				}
			}
			if (bOk) //ドライブレターが一致していたら
			{
				switch (wParam)
				{
					case DBT_DEVICEREMOVECOMPLETE: //CD取り出し時
						APPEVENT_Set(APPEVENT_DEVICEREMOVE, NULL); //すぐにCDアクセスを止めてポーズする。
						break;
					case DBT_DEVICEARRIVAL: //CD挿入時
						APPEVENT_Set(APPEVENT_DEVICECHANGE, NULL); //現在ゲーム中のCDドライブのTOCを再チェックして、現在のゲームと異なればゲームを中断し、新しく挿入されたゲームに切り替える。
					break;
				}
			}
		}
...
//		case WM_STOP_EMULATOR:			APPEVENT_Set(APPEVENT_PAUSE, NULL);					break;
		...
		case WM_SAVE_STATE_51:			APPEVENT_Set(APPEVENT_SAVESTATE_51, NULL);			break;//Kitao追加
		case WM_SAVE_STATE_52:			APPEVENT_Set(APPEVENT_SAVESTATE_52, NULL);			break;//Kitao追加
		case WM_SAVE_STATE_53:			APPEVENT_Set(APPEVENT_SAVESTATE_53, NULL);			break;//Kitao追加
		case WM_SAVE_STATE_54:			APPEVENT_Set(APPEVENT_SAVESTATE_54, NULL);			break;//Kitao追加
		case WM_SAVE_STATE_55:			APPEVENT_Set(APPEVENT_SAVESTATE_55, NULL);			break;//Kitao追加
		case WM_SAVE_STATE_56:			APPEVENT_Set(APPEVENT_SAVESTATE_56, NULL);			break;//Kitao追加
		case WM_SAVE_STATE_57:			APPEVENT_Set(APPEVENT_SAVESTATE_57, NULL);			break;//Kitao追加
		case WM_SAVE_STATE_58:			APPEVENT_Set(APPEVENT_SAVESTATE_58, NULL);			break;//Kitao追加
		case WM_SAVE_STATE_59:			APPEVENT_Set(APPEVENT_SAVESTATE_59, NULL);			break;//Kitao追加
		case WM_SAVE_STATE_60:			APPEVENT_Set(APPEVENT_SAVESTATE_60, NULL);			break;//Kitao追加
		case WM_SAVE_STATE_61:			APPEVENT_Set(APPEVENT_SAVESTATE_61, NULL);			break;//Kitao追加
		case WM_SAVE_STATE_62:			APPEVENT_Set(APPEVENT_SAVESTATE_62, NULL);			break;//Kitao追加
		case WM_SAVE_STATE_63:			APPEVENT_Set(APPEVENT_SAVESTATE_63, NULL);			break;//Kitao追加
		case WM_SAVE_STATE_64:			APPEVENT_Set(APPEVENT_SAVESTATE_64, NULL);			break;//Kitao追加
		case WM_SAVE_STATE_65:			APPEVENT_Set(APPEVENT_SAVESTATE_65, NULL);			break;//Kitao追加
		case WM_SAVE_STATE_66:			APPEVENT_Set(APPEVENT_SAVESTATE_66, NULL);			break;//Kitao追加
		case WM_SAVE_STATE_67:			APPEVENT_Set(APPEVENT_SAVESTATE_67, NULL);			break;//Kitao追加
		case WM_SAVE_STATE_68:			APPEVENT_Set(APPEVENT_SAVESTATE_68, NULL);			break;//Kitao追加
		case WM_SAVE_STATE_69:			APPEVENT_Set(APPEVENT_SAVESTATE_69, NULL);			break;//Kitao追加
		case WM_SAVE_STATE_70:			APPEVENT_Set(APPEVENT_SAVESTATE_70, NULL);			break;//Kitao追加
		case WM_SAVE_STATE_71:			APPEVENT_Set(APPEVENT_SAVESTATE_71, NULL);			break;//Kitao追加
		case WM_SAVE_STATE_72:			APPEVENT_Set(APPEVENT_SAVESTATE_72, NULL);			break;//Kitao追加
		case WM_SAVE_STATE_73:			APPEVENT_Set(APPEVENT_SAVESTATE_73, NULL);			break;//Kitao追加
		case WM_SAVE_STATE_74:			APPEVENT_Set(APPEVENT_SAVESTATE_74, NULL);			break;//Kitao追加
		case WM_SAVE_STATE_75:			APPEVENT_Set(APPEVENT_SAVESTATE_75, NULL);			break;//Kitao追加
		case WM_SAVE_STATE_76:			APPEVENT_Set(APPEVENT_SAVESTATE_76, NULL);			break;//Kitao追加
		case WM_SAVE_STATE_77:			APPEVENT_Set(APPEVENT_SAVESTATE_77, NULL);			break;//Kitao追加
		case WM_SAVE_STATE_78:			APPEVENT_Set(APPEVENT_SAVESTATE_78, NULL);			break;//Kitao追加
		case WM_SAVE_STATE_79:			APPEVENT_Set(APPEVENT_SAVESTATE_79, NULL);			break;//Kitao追加
		case WM_SAVE_STATE_80:			APPEVENT_Set(APPEVENT_SAVESTATE_80, NULL);			break;//Kitao追加
		...
		case WM_LOAD_STATE_51:			APPEVENT_Set(APPEVENT_LOADSTATE_51, NULL);			break;//Kitao追加
		case WM_LOAD_STATE_52:			APPEVENT_Set(APPEVENT_LOADSTATE_52, NULL);			break;//Kitao追加
		case WM_LOAD_STATE_53:			APPEVENT_Set(APPEVENT_LOADSTATE_53, NULL);			break;//Kitao追加
		case WM_LOAD_STATE_54:			APPEVENT_Set(APPEVENT_LOADSTATE_54, NULL);			break;//Kitao追加
		case WM_LOAD_STATE_55:			APPEVENT_Set(APPEVENT_LOADSTATE_55, NULL);			break;//Kitao追加
		case WM_LOAD_STATE_56:			APPEVENT_Set(APPEVENT_LOADSTATE_56, NULL);			break;//Kitao追加
		case WM_LOAD_STATE_57:			APPEVENT_Set(APPEVENT_LOADSTATE_57, NULL);			break;//Kitao追加
		case WM_LOAD_STATE_58:			APPEVENT_Set(APPEVENT_LOADSTATE_58, NULL);			break;//Kitao追加
		case WM_LOAD_STATE_59:			APPEVENT_Set(APPEVENT_LOADSTATE_59, NULL);			break;//Kitao追加
		case WM_LOAD_STATE_60:			APPEVENT_Set(APPEVENT_LOADSTATE_60, NULL);			break;//Kitao追加
		case WM_LOAD_STATE_61:			APPEVENT_Set(APPEVENT_LOADSTATE_61, NULL);			break;//Kitao追加
		case WM_LOAD_STATE_62:			APPEVENT_Set(APPEVENT_LOADSTATE_62, NULL);			break;//Kitao追加
		case WM_LOAD_STATE_63:			APPEVENT_Set(APPEVENT_LOADSTATE_63, NULL);			break;//Kitao追加
		case WM_LOAD_STATE_64:			APPEVENT_Set(APPEVENT_LOADSTATE_64, NULL);			break;//Kitao追加
		case WM_LOAD_STATE_65:			APPEVENT_Set(APPEVENT_LOADSTATE_65, NULL);			break;//Kitao追加
		case WM_LOAD_STATE_66:			APPEVENT_Set(APPEVENT_LOADSTATE_66, NULL);			break;//Kitao追加
		case WM_LOAD_STATE_67:			APPEVENT_Set(APPEVENT_LOADSTATE_67, NULL);			break;//Kitao追加
		case WM_LOAD_STATE_68:			APPEVENT_Set(APPEVENT_LOADSTATE_68, NULL);			break;//Kitao追加
		case WM_LOAD_STATE_69:			APPEVENT_Set(APPEVENT_LOADSTATE_69, NULL);			break;//Kitao追加
		case WM_LOAD_STATE_70:			APPEVENT_Set(APPEVENT_LOADSTATE_70, NULL);			break;//Kitao追加
		case WM_LOAD_STATE_71:			APPEVENT_Set(APPEVENT_LOADSTATE_71, NULL);			break;//Kitao追加
		case WM_LOAD_STATE_72:			APPEVENT_Set(APPEVENT_LOADSTATE_72, NULL);			break;//Kitao追加
		case WM_LOAD_STATE_73:			APPEVENT_Set(APPEVENT_LOADSTATE_73, NULL);			break;//Kitao追加
		case WM_LOAD_STATE_74:			APPEVENT_Set(APPEVENT_LOADSTATE_74, NULL);			break;//Kitao追加
		case WM_LOAD_STATE_75:			APPEVENT_Set(APPEVENT_LOADSTATE_75, NULL);			break;//Kitao追加
		case WM_LOAD_STATE_76:			APPEVENT_Set(APPEVENT_LOADSTATE_76, NULL);			break;//Kitao追加
		case WM_LOAD_STATE_77:			APPEVENT_Set(APPEVENT_LOADSTATE_77, NULL);			break;//Kitao追加
		case WM_LOAD_STATE_78:			APPEVENT_Set(APPEVENT_LOADSTATE_78, NULL);			break;//Kitao追加
		case WM_LOAD_STATE_79:			APPEVENT_Set(APPEVENT_LOADSTATE_79, NULL);			break;//Kitao追加
		case WM_LOAD_STATE_80:			APPEVENT_Set(APPEVENT_LOADSTATE_80, NULL);			break;//Kitao追加
		...
		case WM_OPENROM_BUTTON:			APPEVENT_Set(APPEVENT_OPENROM_BUTTON, NULL);		break;//Kitao追加
		case WM_OPENCUE_BUTTON:			APPEVENT_Set(APPEVENT_OPENCUE_BUTTON, NULL);		break;//Kitao追加
		case WM_OPEN_DEFAULT:			APPEVENT_Set(APPEVENT_OPEN_DEFAULT, NULL);			break;//Kitao追加
		case WM_FOLDER_STATE:			APPEVENT_Set(APPEVENT_FOLDER_STATE, NULL);			break;//Kitao追加
		case WM_CUSTOMIZE_STATE:		APPEVENT_Set(APPEVENT_CUSTOMIZE_STATE, NULL);		break;//Kitao追加
		...
		//case WM_SCREEN_DISABLEAERO:		APPEVENT_Set(APPEVENT_SCREEN_DISABLEAERO, NULL);	break;//Kitao追加。v2.88から非使用
		//case WM_SCREEN_USEAERO:			APPEVENT_Set(APPEVENT_SCREEN_USEAERO, NULL);		break;//Kitao追加。v2.88から非使用
		...
		case WM_SCREEN_FULL60HZ:		APPEVENT_Set(APPEVENT_SCREEN_FULL60HZ, NULL);		break;//Kitao追加
		case WM_SCREEN_FULL120HZ:		APPEVENT_Set(APPEVENT_SCREEN_FULL120HZ, NULL);		break;//Kitao追加
		case WM_SCREEN_SPSCANLINED:		APPEVENT_Set(APPEVENT_SCREEN_SPSCANLINED, NULL);	break;//Kitao追加
		case WM_SCREEN_HRSCANLINEDTV:	APPEVENT_Set(APPEVENT_SCREEN_HRSCANLINEDTV, NULL);	break;//Kitao追加
		case WM_SCREEN_HRSCANLINED:		APPEVENT_Set(APPEVENT_SCREEN_HRSCANLINED, NULL);	break;//Kitao追加
		case WM_SCREEN_HRSCANLINED2:	APPEVENT_Set(APPEVENT_SCREEN_HRSCANLINED2, NULL);	break;//Kitao追加
		case WM_SCREEN_HRSCANLINED3:	APPEVENT_Set(APPEVENT_SCREEN_HRSCANLINED3, NULL);	break;//Kitao追加
		case WM_SCREEN_HRSCANLINEDF:	APPEVENT_Set(APPEVENT_SCREEN_HRSCANLINEDF, NULL);	break;//Kitao追加
		case WM_SCREEN_NONSCANLINED:	APPEVENT_Set(APPEVENT_SCREEN_NONSCANLINED, NULL);	break;//Kitao追加
		case WM_SCREEN_NONSCANLINED2:	APPEVENT_Set(APPEVENT_SCREEN_NONSCANLINED2, NULL);	break;//Kitao追加
		case WM_SCREEN_NONSCANLINEDTV:	APPEVENT_Set(APPEVENT_SCREEN_NONSCANLINEDTV, NULL);	break;//Kitao追加
		...
		case WM_SCREEN_LAGFRAME3:		APPEVENT_Set(APPEVENT_SCREEN_LAGFRAME3, NULL);		break;//v2.88追加
		case WM_SCREEN_LAGFRAME1:		APPEVENT_Set(APPEVENT_SCREEN_LAGFRAME1, NULL);		break;//v2.88追加
		case WM_SCREEN_LAGFRAME0:		APPEVENT_Set(APPEVENT_SCREEN_LAGFRAME0, NULL);		break;//v2.88追加
		...
		case WM_INPUT_2BUTTON_PAD:		APPEVENT_Set(APPEVENT_INPUT_2BUTTON_PAD, NULL);		break;
		case WM_INPUT_3BUTTON_PAD:		APPEVENT_Set(APPEVENT_INPUT_3BUTTON_PAD, NULL);		break;//Kitao追加
		case WM_INPUT_3BUTTON12_PAD:	APPEVENT_Set(APPEVENT_INPUT_3BUTTON12_PAD, NULL);	break;//Kitao追加
		case WM_INPUT_6BUTTON_PAD:		APPEVENT_Set(APPEVENT_INPUT_6BUTTON_PAD, NULL);		break;
...
*/
		...
		case WM_AUDIO_METHOD7SC:		APPEVENT_Set(APPEVENT_AUDIO_METHOD7SC, NULL);		break;//Kitao追加
		case WM_AUDIO_METHOD7SM:		APPEVENT_Set(APPEVENT_AUDIO_METHOD7SM, NULL);		break;//Kitao追加
		case WM_AUDIO_METHOD9SC:		APPEVENT_Set(APPEVENT_AUDIO_METHOD9SC, NULL);		break;//Kitao追加
		case WM_AUDIO_METHOD9SM:		APPEVENT_Set(APPEVENT_AUDIO_METHOD9SM, NULL);		break;//Kitao追加
		case WM_AUDIO_METHOD20:			APPEVENT_Set(APPEVENT_AUDIO_METHOD20, NULL);		break;//Kitao追加
		...
		case WM_AUDIO_HESBIGBUFFER:		APPEVENT_Set(APPEVENT_AUDIO_HESBIGBUFFER, NULL);	break;//Kitao追加
		...
		case WM_CD_CUSTOMIZEINSTALL:	APPEVENT_Set(APPEVENT_CD_CUSTOMIZEINSTALL, NULL);	break;//Kitao追加
		...
		case WM_CD_VIEWACCESS:			APPEVENT_Set(APPEVENT_CD_VIEWACCESS, NULL);			break;//Kitao追加
		...
		case WM_IMPROVE_CUTTRACKNOISE:	APPEVENT_Set(APPEVENT_IMPROVE_CUTTRACKNOISE, NULL);	break;//Kitao追加
		case WM_IMPROVE_GRADIUS2:		APPEVENT_Set(APPEVENT_IMPROVE_GRADIUS2, NULL);		break;//Kitao追加
		case WM_IMPROVE_TOKIMEMO:		APPEVENT_Set(APPEVENT_IMPROVE_TOKIMEMO, NULL);		break;//Kitao追加
		case WM_IMPROVE_TENKA1552:		APPEVENT_Set(APPEVENT_IMPROVE_TENKA1552, NULL);		break;//Kitao追加
		case WM_IMPROVE_MEGATEN:		APPEVENT_Set(APPEVENT_IMPROVE_MEGATEN, NULL);		break;//Kitao追加
		case WM_IMPROVE_DARIUSCD:		APPEVENT_Set(APPEVENT_IMPROVE_DARIUSCD, NULL);		break;//Kitao追加
		case WM_IMPROVE_DARIUS26:		APPEVENT_Set(APPEVENT_IMPROVE_DARIUS26, NULL);		break;//Kitao追加
		case WM_IMPROVE_SORCERIANOP:	APPEVENT_Set(APPEVENT_IMPROVE_SORCERIANOP, NULL);	break;//Kitao追加
		case WM_IMPROVE_SORCERIANBGM:	APPEVENT_Set(APPEVENT_IMPROVE_SORCERIANBGM, NULL);	break;//Kitao追加
		case WM_IMPROVE_RTYPECD:		APPEVENT_Set(APPEVENT_IMPROVE_RTYPECD, NULL);		break;//Kitao追加
		...
		case WM_SPRITEBG_BUTTON:		APPEVENT_Set(APPEVENT_SPRITEBG_BUTTON, NULL);		break;//Kitao追加
		case WM_STARTSPRITE_PRE:		APPEVENT_Set(APPEVENT_STARTSPRITE_PRE, NULL);		break;//Kitao追加
		case WM_POWERON_EFFECT:			APPEVENT_Set(APPEVENT_POWERON_EFFECT, NULL);		break;//Kitao追加
		case WM_STRIKEBALL:				APPEVENT_Set(APPEVENT_STRIKEBALL, NULL);			break;//Kitao追加
		...
		case WM_MENU_MESSAGETIME100:	APPEVENT_Set(APPEVENT_MENU_MESSAGETIME100, NULL);	break;//Kitao追加
		case WM_MENU_MESSAGETIME80:		APPEVENT_Set(APPEVENT_MENU_MESSAGETIME80, NULL);	break;//Kitao追加
		case WM_MENU_MESSAGETIME60:		APPEVENT_Set(APPEVENT_MENU_MESSAGETIME60, NULL);	break;//Kitao追加
		case WM_MENU_MESSAGETIME40:		APPEVENT_Set(APPEVENT_MENU_MESSAGETIME40, NULL);	break;//Kitao追加
		case WM_MENU_MESSAGETIME20:		APPEVENT_Set(APPEVENT_MENU_MESSAGETIME20, NULL);	break;//Kitao追加
		...
		case WM_SPEED_MAXFASTCD:		APPEVENT_Set(APPEVENT_SPEED_MAXFASTCD, NULL);		break;//Kitao追加
		...
		case WM_SSHOT_SPSCANLINED:		APPEVENT_Set(APPEVENT_SSHOT_SPSCANLINED, NULL);		break;//Kitao追加
		case WM_SSHOT_HRSCANLINEDTV:	APPEVENT_Set(APPEVENT_SSHOT_HRSCANLINEDTV, NULL);	break;//Kitao追加
		case WM_SSHOT_HRSCANLINED:		APPEVENT_Set(APPEVENT_SSHOT_HRSCANLINED, NULL);		break;//Kitao追加
		case WM_SSHOT_HRSCANLINED2:		APPEVENT_Set(APPEVENT_SSHOT_HRSCANLINED2, NULL);	break;//Kitao追加
		case WM_SSHOT_HRSCANLINED3:		APPEVENT_Set(APPEVENT_SSHOT_HRSCANLINED3, NULL);	break;//Kitao追加
		case WM_SSHOT_HRSCANLINEDF:		APPEVENT_Set(APPEVENT_SSHOT_HRSCANLINEDF, NULL);	break;//Kitao追加
		case WM_SSHOT_NONSCANLINED:		APPEVENT_Set(APPEVENT_SSHOT_NONSCANLINED, NULL);	break;//Kitao追加
		case WM_SSHOT_NONSCANLINED2:	APPEVENT_Set(APPEVENT_SSHOT_NONSCANLINED2, NULL);	break;//Kitao追加
		case WM_SSHOT_NONSCANLINEDTV:	APPEVENT_Set(APPEVENT_SSHOT_NONSCANLINEDTV, NULL);	break;//Kitao追加
	...
	//v2.49追加。従来の対処方法でスクリーンセーバーが抑止されななかった環境にも対応。
	case WM_SYSCOMMAND:
...
WINMAIN_SetCaption(
	const char*		pCaption,
	BOOL			bUpdate) //_Caption(現在表示中の見出しの履歴変数)を更新するならTRUE。v2.84追加
{
	if (pCaption)
	{
	...
	else
	{
		...
		if (bUpdate)
			strcpy(_Caption, APP_GetAppName());
	}

	MAINBOARD_SetCDAccess(FALSE); //CDアクセス中の表示を非表示状態に戻す。v2.84追加
}
	...
	int			nCmdShow)
{
	...
	WINMAIN_SetPriority(ABOVE_NORMAL_PRIORITY_CLASS); //v2.04更新
