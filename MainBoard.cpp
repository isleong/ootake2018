...
・バックアップRAMの読み出し時に、先頭の2KBだけを読みだすようにした。Mednafenと
　相互互換。v2.88
...
static Uint32		_SysTime; //Kitao追加。システムリセット後の経過時間。1/60秒単位。
static Uint32		_SysTimeCheck; //Kitao追加
...
static BOOL			_bWorldStadium91; //Kitao追加。ワールドスタジアム'91起動時にTRUE。ストライクとボールの表示をスワップする際に利用。v2.70
static BOOL			_bWorldStadium; //Kitao追加。ワールドスタジアム(初代)起動時にTRUE。ストライクとボールの表示をスワップする際に利用。v2.70
static BOOL			_bPowerLeague34593; //Kitao追加。パワーリーグ３,４,５,'93起動時にTRUE。ストライクとボールの表示をスワップする際に利用。v2.70
static BOOL			_bPowerLeague2; //Kitao追加。パワーリーグ２起動時にTRUE。ストライクとボールの表示をスワップする際に利用。v2.70
static BOOL			_bNaxatStadium; //Kitao追加。ナグザットスタジアム起動時にTRUE。ストライクとボールの表示をスワップする際に利用。v2.70
static BOOL			_bKoregaPro; //Kitao追加。これがプロ野球'89，これがプロ野球'90起動時にTRUE。ストライクとボールの表示をスワップする際に利用。v2.70
...
static BOOL			_bUpdateFullScreen = TRUE; //Kitao追加。通常は常にTRUEにしておく。FALSEの場合screenUpdate()でフルスクリーン時に画面外を含めたクリア動作（ゲーム画面も一瞬暗くなる）をしない。音量変更時の表示などで画面をちらつかせたくないときに利用。v2.77
...
static BOOL			_bCDAccess = FALSE; //CDアクセス中ならTRUE。Option画面の周波数測定で使われるので必ず起動時はFALSEに。v2.84追加
		...
		switch (mpr)
		{
			...
			case 0xFF:
				switch (addr & 0x1C00)
				{
					...
					case 0x0800: // PSG
						//if (gCPU_Transfer != 0) return 0x00; //転送命令からのアクセスだった場合。v2.81追加
...
setGameSetting()
{
	...
	_SysTimeCheck = 0; //起動時に自動的な動作をさせる場合、これを設定する。自動動作をさせない場合(通常)は0。v1.15追加
	_bKonamiStereo = FALSE;
	_bGradiusII = FALSE;
	_bWorldStadium91 = FALSE;
	VDC_SetWorldStadium91(FALSE);
	_bWorldStadium = FALSE;
	_bPowerLeague34593 = FALSE;
	_bPowerLeague2 = FALSE;
	_bNaxatStadium = FALSE;
	_bKoregaPro = FALSE;
	...
	if (_RomSize == 1048576) //8M
	{
		if ((_pRom[0x30] == 0x3F)&&
			...
			{	//起動時に１ボタンを押しっぱなしにして自動的にステレオ起動する
				...
				_SysTimeCheck = 3*60; //１ボタンを離すフレーム。起動から３秒後
			}
		if ((_pRom[0x10] == 0x20)&&
			(_pRom[0x11] == 0x72)&&
			(_pRom[0x12] == 0xE2)&&
	...
	if (_RomSize == 786432) //6M
	{
		if ((_pRom[0x10] == 0x88)&&
			...
			{
				_bPowerLeague34593 = TRUE; //ストライクとボールの表示をスワップする際に使用。v2.70
	...
	if (_RomSize == 524288) //4M
	{
		...
		if ((_pRom[0x20] == 0xA9)&&
			...
			(_pRom[0x25] == 0xFF)) //バルンバ
			{
				...
				else //最初から３ボタンパッドをつないでいた場合
				{
					...
					{
						JOYPAD_SetChange3to12(FALSE); //通常の３ボタンパッド設定にする。
						PRINTF("Button [IIIr]->[RUN] [IIIs]->[SELECT].");
					}
				}
				//if (!APP_GetUseThreeButtonR()) //起動時２ボタンパッドをつないでいた場合
				//{
		...
		if ((_pRom[0x30] == 0x20)&&
			...
			(_pRom[0x33] == 0xFF)&&
			(_pRom[0x34] == 0x1F)&&
			(_pRom[0x35] == 0x73)) //ナグザットスタジアム
				_bNaxatStadium = TRUE; //ストライクとボールの表示をスワップする際に使用。v2.70
		if ((_pRom[0x30] == 0xFF)&&
			(_pRom[0x31] == 0x9A)&&
			(_pRom[0x32] == 0x20)&&
			(_pRom[0x33] == 0xBE)&&
...
*/
	...
	if (_RomSize == 393216) //3M
	{
		...
		if ((_pRom[0x10] == 0x05)&&
			...
			(_pRom[0x13] == 0xD7)&&
			(_pRom[0x14] == 0x80)&&
			(_pRom[0x15] == 0x03)) //パワーリーグ２
				_bPowerLeague2 = TRUE; //ストライクとボールの表示をスワップする際に使用。v2.70
		if (((_pRom[0x30] == 0x02)&&
			 (_pRom[0x31] == 0x14)&&
			 (_pRom[0x32] == 0xD4)&&
			 (_pRom[0x33] == 0x78)&&
		...
		/*if ((_pRom[0x30] == 0x22)&&
			...
			(_pRom[0x33] == 0x22)&&
			(_pRom[0x34] == 0x02)&&
			(_pRom[0x35] == 0x22)) //ファイヤープロレスリング
				JOYPAD_SetChange3to12(TRUE); //３ボタンパッドをつないだときにIIIrボタンにI+IIボタンを割り当てる。＆IIIsボタンはIIIrボタンになる。
		*/ //I+IIボタン同時押しによる隠し技を使い易く。操作の混乱になるかもなので現状はカット。v2.87
	}
	if (_RomSize == 262144) //2M
	{
		...
		if ((_pRom[0x40] == 0x31)&&
			...
			{
				_bWorldStadium91 = TRUE; //ストライクとボールの表示をスワップする際に使用。v2.70
		...
		if ((_pRom[0x40] == 0x60)&&
			...
			(_pRom[0x43] == 0xD0)&&
			(_pRom[0x44] == 0x29)&&
			(_pRom[0x45] == 0xA5)) //ワールドスタジアム
				_bWorldStadium = TRUE; //ストライクとボールの表示をスワップする際に使用。v2.70
		if (((_pRom[0x30] == 0x43)&&
			 (_pRom[0x31] == 0x80)&&
			 (_pRom[0x32] == 0x85)&&
			 (_pRom[0x33] == 0x12)&&
			 (_pRom[0x34] == 0xD8)&&
			 (_pRom[0x35] == 0x20))|| //パワーリーグ
			(strstr(pGameFileNameBuf,"パワーリーグ オールスター GOLD HuCARD (J)") != NULL)|| //動作未確認
			(strstr(pGameFileNameBuf,"World Class Baseball (U)") != NULL)) //動作未確認
				VDC_SetPowerLeague(TRUE); //ストライクとボールの表示をスワップする際に使用。v2.70
		if ((_pRom[0x30] == 0x00)&&
			(_pRom[0x31] == 0xA9)&&
			(_pRom[0x32] == 0x20)&&
			(_pRom[0x33] == 0x85)&&
			(_pRom[0x34] == 0x01)&&
			(_pRom[0x35] == 0xA9)) //これがプロ野球'89，これがプロ野球'90
				_bKoregaPro = TRUE; //ストライクとボールの表示をスワップする際に使用。v2.70
		if ((_pRom[0x30] == 0xE0)&&
			(_pRom[0x31] == 0x20)&&
			(_pRom[0x32] == 0xCA)&&
			(_pRom[0x33] == 0x9A)&&
		...
		if ((_pRom[0x30] == 0x00)&&
			...
			(_pRom[0x33] == 0x97)&&
			(_pRom[0x34] == 0x29)&&
			(_pRom[0x35] == 0xAD)) //ロードランナー（パックインビデオ版）
			{
				JOYPAD_ConnectMultiTap(FALSE);//マルチタップを無効に。付けているとEDITモード時のカーソルの動きが微妙に悪くなる。v2.83
				_bAutoDisconnectMultiTap = TRUE;//過去バージョンのステートセーブを読み込んだ場合も無効をキープし続けるために必要。
			}
		if (((_pRom[0x30] == 0x1A)&&
			 (_pRom[0x31] == 0x78)&&
...
{
	_bGradiusII = TRUE;
	_SysTimeCheck = _SysTime + 1*60; //コントローラ２の下キーを離すフレーム。現在のフレームから１秒後
}
	...
	const char*		pGameName)
{
	...
	char	fn[MAX_PATH+1];
	char*	pOtherSysOpenName; //v2.07追加
	...
	{
		if (!APP_GetCDGame()) //システムカード読み込み時以外の場合、エラーメッセージを表示。v2.77更新
		{
			sprintf(buf, "Couldn't Open \"%s\".    ", APP_GetGameFilePathName());
			MessageBox(WINMAIN_GetHwnd(), buf, "Ootake", MB_OK);
		}
	...
	if (bBad)
	{
		if (APP_GetCDGame())
			...
			strcpy(fn, APP_GetGameFileNameBuf());
		strcat(fn, ".pce");
		sprintf(buf, "\"%s\"\n"
					 ...
					 " 適切なTG16規格に変換したファイルをmb128フォルダへ出力します。", fn);
		if (MessageBox(WINMAIN_GetHwnd(), buf, "Ootake", MB_YESNO) == IDYES)
			MAINBOARD_TG16BitConvert(NULL, APP_GetCDGame());
		return FALSE;
	}
...
*/
	...
	if ((_pRom[0x00] == 0x4C)&&
		...
		(_pRom[0x03] == 0x4C)&&
		(_pRom[0x0E] == 0xE9)&&
		(_pRom[0x0F] == 0x4C))	//ゲームエクスプレスCDカード
			_bSystemCard = TRUE;
	...
	if (_RomSize == 524288) //4M
	{
		...
		if (bBad)
		{
			sprintf(buf, "\"%s\"\n"
						 ...
						 " 適切なサイズ（3Mbit）に変換したファイルをmb128フォルダへ出力します。", APP_GetGameFileNameBuf());
			if (MessageBox(WINMAIN_GetHwnd(), buf, "Ootake", MB_YESNO) == IDYES)
	...
	_pBackupRam = (Uint8*)malloc(0x2000); //実機は2KBだが、YAMEやMagicEngine等古いエミュレータとの互換維持のため8KB確保。v2.88からはMednafenとの互換を優先するためセーブ(bram)ファイルは2KBに変更した。ステートセーブ＆ロードの互換維持のため、内部メモリは引き続き8KB確保しておく。8KBあることでバックアップの自動履歴保存等で使えそうだが必要ないかな。
	...
	_bCDAccess = FALSE; //Kiao追加。CDアクセス中ならTRUEになる。v2.84
	...
	{
		pOtherSysOpenName = APP_ChangeToOtherSysCard1();
		...
		if (pOtherSysOpenName != NULL)
			_RomMask = CART_LoadCartridge(pOtherSysOpenName, &_pRom, &_RomSize); //イメージファイルを読み込み直す
		else//イメージファイルを読み込めなかった場合
			_RomMask = 0;
		if (_RomMask == 0)
		{	//Old SystemCardのイメージファイル設定をクリア
			APP_ClearOtherSysCard1();
			return FALSE; //失敗
		}
		//正常に開けた
		sprintf(buf, "\"Old System Card\" Set OK.  %s Juuouki (J)", buf2);
		PRINTF(buf);
	}

	//ゲームエクスプレスCDカード用ゲームの場合。v2.83
	if ((APP_GetCDGame())&&(CDROM_GetGamesExpress()))
	{
		pOtherSysOpenName = APP_ChangeToOtherSysCard2();
		switch (CDROM_GetCDInstall())
		{
			case 1: //データトラックインストール済み
				strcpy(buf2, "Icd:"); //※buf2=最大4文字
		...
		if (pOtherSysOpenName != NULL)
			...
			_RomMask = 0;
		if (_RomMask == 0)
		{	//ゲームエクスプレスCDカードのイメージファイル設定をクリア
			APP_ClearOtherSysCard2();
			return FALSE; //失敗
		}
		//正常に開けた
		sprintf(buf, "\"Games Express CD Card\" Set OK.  %s %s", buf2, TOCDB_GetGameTitle());
		PRINTF(buf);
	}
	...
	const char*		pathName)
{
	...
	//           こうすることで、間違えてステートロードしてしまった時でも、すぐ終了すれば元のバックアップラムファイルは残ってくれる。＆バックアップの必要のないゲームはファイルが不要になる。
	if (memcmp(_pBackupRam, &_FirstBackupRam, 0x800) != 0) //更新されていれば
	{
		bram = fopen(pathName, "wb");
		if (bram != NULL && _pBackupRam != NULL)
		{
			fwrite(_pBackupRam, sizeof(Uint8), 0x800, bram); //実機容量と同じ先頭「2KB」のみを保存。v2.88。2047バイトしか保存していなかった不具合を修正。v2.90
			fclose(bram);
	...
	const char*		pathName)
{
	...
	if (bram != NULL && _pBackupRam != NULL)
	{
		fread(_pBackupRam, sizeof(Uint8), 0x800, bram); //実機容量と同じ先頭「2KB」のみを読込。v2.88。2047バイトしか保存していなかった不具合を修正。v2.90
		fclose(bram);
		//Kitao追加。バックアップラムの最初の状態を退避しておく。終了時に更新された形跡があったときだけ、バックアップラムを保存するようにした。
		memcpy(&_FirstBackupRam, _pBackupRam, 0x2000);
		return TRUE;
...
MAINBOARD_AdvanceFrame() //Kitao更新。ストレッチのときも非ストレッチのときもここを使うようにした。
{
	...
	Sint32		thirdEncodeLine; //v2.70追加
	char		buf[100]; //v2.84追加
	...
	if (_SysTime != 0xFFFFFFFF)
		if (++_SysTime == _SysTimeCheck)
		{
	...
	thirdEncodeLine = MAX_SCANLINE/2+MAX_SCANLINE/4+1; //v2.70追加。PSG割り込み再現のための分割エンコード３回目の位置。
	if (_bNaxatStadium) //ナグザットスタジアムでストライクとボールをスワップ表示。スワップするために境界線の少し下までエンコード。v2.70
		if (APP_GetStrikeBallSwap()) thirdEncodeLine += 13;
	for (_ScanLine=0; _ScanLine<=MAX_SCANLINE-1; _ScanLine++) //Kitao更新。実記並のパフォーマンスを出すには、vblank前後にもっとCPUパワーが必要なため、１フレームを263ラインぶんと仮定した。
	{
		...
		if (psgQuality == 4)
		{
				...
				(_ScanLine == thirdEncodeLine))
			{
				...
				if (_ScanLine == MAX_SCANLINE/2)
					t -= 4;
				if (_ScanLine == thirdEncodeLine)
					t -= 8-2; //-2は処理落ち防止のため（最終描画転送処理があるぶん）。-1だと省電力モードになっているPCで処理落ちあり。意外なことに-1より-2のほうが音色が再現度高いかも（2:2:1:3の割合での時間分割が有効）。v2.91更新
				//if (t <= 2) PRINTF("%d", t); //test
				if (SCREEN_GetVSync120Hz())
				{
					if ((t == 0)||(t == 1)) //2ms以上(1.1ms以上)経過していた場合は待たない。タイマーカウンタのオーバーフローも考えて、この範囲のときだけSleepをおこなう。
						Sleep(1); //処理落ち防止のため1ウェイト固定
				}
				else //通常(60Hz)時
				{
					if (t <= 3) //4ms以上(3.1ms以上)経過していた場合は待たない。1/240s=4.17ms。VRAM転送＆描画処理を後半にやらねばならないので、ここを3ms以下で処理できたPCだけ処理を行う。
					{
		...
		else if (psgQuality == 2)
		{
			if (_ScanLine == MAX_SCANLINE/2)
			{
				...
				if (SCREEN_GetVSync120Hz())
				{
					if (t <= 2) //3ms以上(2.1ms以上)経過していた場合は待たない。
					{
						t = 4-t-1; //4…１コマの時間約8msの1/2。-1は処理落ち防止のため（最終描画転送処理があるぶん）。
						if ((t >= 1)&&(t <= 3)) //タイマーカウンタのオーバーフローも考えて、この範囲のときだけSleepをおこなう。
							Sleep(t);
					}
				}
				else //通常(60Hz)時
				{
					if (t <= 6) //7ms以上(6.1ms以上)経過していた場合は待たない。
					{
						t = 8-t-1; //8…１コマの時間約16msの1/2。-1は処理落ち防止のため（最終描画転送処理があるぶん）。
						if ((t >= 1)&&(t <= 7)) //タイマーカウンタのオーバーフローも考えて、この範囲のときだけSleepをおこなう。
							Sleep(t);
					}
				}
			}
		...
		(_BigSoundCounter == 0)) //Kitao追加。ビッグサウンドバッファモードのときは6回に1度だけ画面描画をおこなう。
	{
		...
		SCREEN_Blt(_ScreenBuf, 0, encodeLine, _TvW, 240-encodeLine+_ShowOverscanBottom, 1); //Kitao更新。VCEの解像度(tvW)で表示するようにした。VBlank待ちも行われる
	...
	//CDアクセスの表示。この位置で表示することで、Windowsが垂直帰線期間になっている空いた時間にCDアクセス表示ができる。v2.84追加
	//Sleep(4);//開発用PCでどのぐらい処理に余裕があるかtest用。
	if (!_bFullScreen && APP_GetViewCDAccess()) //現状はWindowモードのみ
	{
		if ((CDROM_GetReadByteCount() > 10)||(CDROM_GetCDAccessCount() > 0)) //ReadByteCountが10以下の場合は、CDDA再生中にサブチャンネルを読んでいる場合アクセス表示が激しすぎるので表示しない。v2.85
		{
			if (!_bCDAccess) //現在アクセス中の表示をしていない場合	
			{
				strcpy(buf, WINMAIN_GetCaption());
				strcat(buf, ":llll");
				WINMAIN_SetCaption(buf, FALSE);
				_bCDAccess = TRUE;
			}
		}
		else
		{
			if (_bCDAccess) //現在アクセス中の表示をしている場合	
			{
				strcpy(buf, WINMAIN_GetCaption());
				WINMAIN_SetCaption(buf, FALSE); //_bCDAccessもFALSEになる
			}
		}
	}
...
//Kitao追加。１フレーム処理をシミュレート。ディスプレイ周波数算出時に使用。v2.79
void
MAINBOARD_AdvanceFrameSim()
{
	Sint32 		t;
	Sint32		startLine = VDC_GetTvStartLine();
	Sint32		encodeLine;
	Sint32		encodeLineH;
	Sint32		psgQuality;
	Sint32		thirdEncodeLine;

	encodeLine = 16-_ShowOverscanTop;
	thirdEncodeLine = MAX_SCANLINE/2+MAX_SCANLINE/4+1; //v2.70追加。PSG割り込み再現のための分割エンコード３回目の位置。
	for (_ScanLine=0; _ScanLine<=MAX_SCANLINE-1; _ScanLine++)
	{
		// おおよそ1/240秒単位でPSGのレジスタ更新を行い(早めに更新されてしまわないように待ち)、音色を実機に近づける処理
		psgQuality = APP_GetPsgQuality();
		if (psgQuality == 4)
		{
			if ((_ScanLine == MAX_SCANLINE/4)||
	...
	// 最後(最下部)のエンコードを行い、画面表示を更新する。
	SCREEN_Blt(_ScreenBuf, 0, encodeLine, _TvW, 240-encodeLine+_ShowOverscanBottom, 1);
}
	...
	Sint32	showOverscanRight)
{
	...
	if (_bUpdateFullScreen)
		if ((_bFullScreen)|| //フルスクリーン表示の場合、メニュー表示などではみだしてしまった部分が残らないようにクリアする。
			...
			(showOverscanLeft < _ShowOverscanLeft)||(showOverscanLeft < _ShowOverscanLeft)) //または、ウィンドウモードで描画するソースのほうが小さい場合
				SCREEN_Clear(0,0,0);
...
MAINBOARD_ScreenUpdate(
	BOOL	bVsync,				//VSyncを行うかどうか。通常はbVsyncをTRUEにして呼ぶ。特にDirect3DはVsync待ちを行わないと描画も行われないので注意。
	BOOL	bFullScreenRefresh) //フルスクリーン時に画面外も含めてクリア動作(ゲーム画面も一瞬黒くなる)をするか。通常はbVsyncをTRUEにして呼ぶ。クリア動作のちらつきをさせたくないシーンのときFALSEで呼ぶ。v2.77
{
	if (!bVsync) _UpdateVsync=2;
	if (!bFullScreenRefresh) _bUpdateFullScreen=FALSE;
	screenUpdate(_ShowOverscanTop,_ShowOverscanBottom,_ShowOverscanLeft,_ShowOverscanRight);
	_UpdateVsync = 1; //通常時のために常に1に戻しておく。
	_bUpdateFullScreen = TRUE; //通常時のために常にTRUEに戻しておく。
}
...
//Kitao追加。ゲーム画面を初期化し、画面の転送更新を行う。この関数はゲーム起動前にも利用可。Screen.c から利用。v2.77
void
	...
	BOOL	bPause)
{
	...
	if (bPause)
		pause_sub(TRUE); //ポーズ時は、音はノイズが出ないようにいち早く止めておく
	pause_sub2(bPause);
	if (!bPause)
		pause_sub(FALSE); //ポーズ解除は、音はノイズが出ないように一番最後に音を発生させる
}
...
MAINBOARD_TG16BitConvert(
	char*	pGameName,	//ROMイメージを読み込むならファイルパス名を入れて呼ぶ。読み込み済みで必要ない場合NULL。
	BOOL	bCDGame)	//CDゲーム（システムカードのROM）を起動状態ならTRUEで呼ぶ。
{
	Uint32	addr;
	Sint32	offset;
	Uint8	src;
	...
	//ROMデータ読み込み
	if (pGameName != NULL)
		if ((_RomMask = CART_LoadCartridge(pGameName, &_pRom, &_RomSize)) == 0) return; //読み込みエラーの場合返る
	...
	strcat(fileName, "mb128");
	CreateDirectory(fileName, NULL);//mb128ディレクトリがない場合作る
	strcat(fileName, "\\");
	if (bCDGame)
		strcat(fileName, "SYSCARD");
	else
	...
	if (fp != NULL)
	{
		...
		fclose(fp);
		sprintf(buf, "Conversion was completed.\n\n"
					 "Correct \"%s\" was saved into the \"mb128\" folder.    \n"
					 "The \"mb128\" folder is opened.    ", APP_ExtractFileName(fileName));
		MessageBox(WINMAIN_GetHwnd(), buf, "Ootake", MB_OK);
		APP_OpenMB128Folder();
...
MAINBOARD_OverDumpedConvert()
{
	...
	if (fp != NULL)
	{
		...
		sprintf(fileName, "Conversion was completed.\n\n"
						  "Correct \"%s.pce\" was saved into the \"mb128\" folder.    \n"
						  "The \"mb128\" folder is opened.    ", APP_GetGameFileNameBuf());
...
{
	return _bUpdateFullScreen;
}

//CDアクセス中の表示を非表示状態に戻す。v2.84追加
void
MAINBOARD_SetCDAccess(
	BOOL	bCDAccess)
{
	_bCDAccess = bCDAccess;
}

//Kitao追加。CDアクセス表示をリセット(FALSE)にする。v2.84
void
	...
	FILE*		p)
{
	...
	int			i;
	Uint32		offset;
	BOOL		bHalfMode = APP_GetHalfMode();
	Sint32		VDCWidth;
	...
	SAVE_V(_bStretched);
	SAVE_V(_bVStretched);
	SAVE_V(bHalfMode); //過去バージョンで使っていた。現状はダミー。v2.78記
	...
	SAVE_V(_SysTime); //v1.15追加。ゲーム起動からの経過時間(単位はフレーム)
	...
	FILE*		p)
{
	...
	//PRINTF("Version %d",_StateVersion); //test用
	//return FALSE; //test用
	...
	if (_StateVersion >= 20) //Kitao追加。v0.94以降なら
	{
		...
		LOAD_V(bHalfMode);
		//if (bHalfMode != APP_GetHalfMode()) //現在のモードと異なる場合。※v1.53からHalfModeは反映しないようにした
		//	APP_SetHalfMode(bHalfMode);
...
//			long so = ftell(p);
//			char s[100];
//			sprintf(s,"v%d SysTime Offset = %X", _StateVersion, so);
//			MessageBox(WINMAIN_GetHwnd(), s, "Test", MB_OK);
		}
		LOAD_V(_SysTime); //ゲーム起動からの経過時間(単位はフレーム)
	}
	else
		_SysTime = 0xFFFFFFFF;
	if (_SysTime == 0xFFFFFFFF)
	...
	if (_StateVersion >= 27) //Kitao追加。v1.15以降のセーブファイルなら
	{
		LOAD_V(_SysTimeCheck);
		if (_SysTime == 0) //「古いバージョンでスタートしたレジュームファイル」をロードした場合。v2.63
			_SysTimeCheck = 0;
	...
	if ((_bKonamiStereo)&&(APP_GetAutoStereo()))
		if (_SysTime < _SysTimeCheck) //チェックポイント未到達の場合
			bKonamiStereo = TRUE;
	...
	if ((((APP_GetFullMagnification() == 0)&&((_ScreenWidth != 640)||(_ScreenHeight != 480)))||(APP_GetFullMagnification() == 2))&&
		(_bFullScreen)&&(APP_GetFullStretched(FALSE))&&((APP_GetScanLineType() >= 2)&&(APP_GetScanLineType() <= 4)))
