...
ADPCM_Reset()
{
	...
	//PRINTF("Repeat=%d Stream=%d LengthCount=%X", _bRepeat, _bStream, _LengthCount);//test用
	...
	_WriteAddr = 0;
	_LengthCount = 0;
	_PlayLength = 0; //v2.83追加
	_PlayedSampleCount = 0; //v2.83追加
	...
	_DecodeBuffer = 0; //v2.83追加
	_bLowNibble	= TRUE; //v2.83追加
	_Phase = 0; //v2.83追加
	_SampleFreq = 16000.0; //v2.83追加。ガリバーボーイで必要
	set_DP(); //v2.38追加。_DPがセットされる
	_CDDAAjustCount = 0; //v2.83追加
...
void
ADPCM_SetReadAddr(
	Uint8	readBuffer) //readBuffer=空読みする回数
{
/*
{
	char s[100];
...
*/
	...
	_ReadBuffer = readBuffer; //空読み回数をセット。v2.83更新。CD麻雀美少女中心派で１回だけの空読みが必要。ひろさん、情報有難うございました。
	_ReadAddr = _Addr;
...
*/
	...
	//ストリーム再生かどうかをチェックする。v2.24更新。AYAで音の途切れ解消
	//PRINTF("bRepeat=%d , WriteAddr=%X , WriteBuffer=%d , ReadAddr=%X , LengthCount=%X",_bRepeat, (int)_WriteAddr, _WriteBuffer, _ReadAddr, _LengthCount);//test用
	if ((_WriteAddr == 0)||(_WriteAddr == 0x8000)||((_WriteAddr & 0x1FFF) == 0x1FFF)) //v2.83追加。この条件でパステルライム(非ストリーム再生)の画像乱れ解消。
		if ((((_ReadAddr + _LengthCount) & 0x1FFF) == 0x1FFF)|| //AYAで音の途切れ解消。ラストハルマゲドン,エフェラ＆ジリオラ,ホームズ１２。
			((_ReadAddr == 0)&&(_LengthCount == 0x8000))) //聖戦士伝承
				_bStream = TRUE; //ストリーム再生
}
...
ADPCM_SetLength()
{
	...
	//PRINTF("SampleFreq=%d , Read=%X , Write=%X , Length=%X ,Repeat=%d, WriteBuffer=%d", (int)_SampleFreq, (int)_ReadAddr, (int)_WriteAddr, (int)_LengthCount, (int)_bRepeat, (int)_WriteBuffer);//test用
...
//v2.83追加。現在溜まっている再生予定ADPCMデータのバイト数。
Uint32
ADPCM_GetRemainingLength()
{
	return _PlayLength - _PlayedSampleCount;
}
	...
	BOOL	bRepeat)
{
	...
	//再生中にリピートFALSEなら再生を止める。スチームハーツでRUNボタンセリフ飛ばし時に必要。v2.83追加
	if (_bPlay && !_bRepeat && !_bStream)
		_bPlay = FALSE;

	//停止中にリピートTRUEなら再生を開始。v2.83追加
	if (!_bPlay && _bRepeat)
		_bPlay = TRUE;
}
...
{
	//PRINTF("bRepeat=%d , WriteAddr=%X , WriteBuffer=%d , ReadAddr=%X , LengthCount=%X",_bRepeat, (int)_WriteAddr, _WriteBuffer, _ReadAddr, _LengthCount);//test用
	//PRINTF("Stream=%d , PlayAddr=%X , PlayLength=%X , _PlayedSampleCount=%X", _bStream, (int)_PlayAddr, (int)_PlayLength, (int)_PlayedSampleCount);//test用
	//PRINTF("PlayLength=%X , PlayLength-_PlayedSampleCount=%X", (int)_PlayLength, (int)(_PlayLength-_PlayedSampleCount));//test用

	if (bPlay)
	{
		_bPlay = TRUE; //v2.83追加。bPlayがFALSEでも_bPlayがTRUEなら再生は続けられる。
	}
	else //bPlay==FALSEで呼ばれた場合
	{		
		if (_bStream) //※ストリーム再生(((_ReadAddr + _LengthCount) & 0x1FFF) == 0x1FFF)だった場合は、すぐのストップはせずに割込み位置まで再生する。ラストハルマゲドン,エフェラ＆ジリオラ,ホームズ１２。
		{
			if (_PlayLength - _PlayedSampleCount > 0x8000) //ストリーム再生でも残りが0x8000以上の状態(キャンセルが入った)の場合はすぐにストップ。
				_bPlay = FALSE; //エフェラ＆ジリオラのOPデモでRUNを押してスキップした時に必要。
			else
				return; //何もせずリターン。再生を続ける
		}
	}
	...
	if (_bPlay)
	{
		...
		else //再生中にbPlay=FALSEで呼ばれた場合。割り込みを起こすアドレスの更新のみを行うようにした。v2.83
		{
			//PRINTF("bPlay=%d Repeat=%d Stream=%d LengthCount=%X PlayLength=%X PlSmpCount=%X", bPlay, _bRepeat, _bStream, _LengthCount, (int)_PlayLength, (int)_PlayedSampleCount);//test用
			if (_bRepeat &&(_LengthCount >= 0x8000)&&(_LengthCount <= 0x80FF))
				_PlayHalfAddr = _PlayAddr + 0x85; //ガリバーボーイで133byte再生毎に割り込みを起こす。v2.83追加。ひろさんに掲示板で頂いた情報が大きなヒントになりました。ありがとうございます！
			else
				_PlayHalfAddr = _PlayAddr + (Uint16)((_LengthCount + 1) >> 1);
		}
	}
	else 
	{
...
makeSample()
{
	...
	BOOL	bWait = FALSE; //v2.83追加

	//PRINTF("Kekka=%X, %X, %X, %X",(int)_PlayAddr, _PlayHalfAddr, _PlayedSampleCount, _PlayLength);//test用
	...
	for (j=1; j<=(Sint32)ADPCM_OVERSAMPLRATE; j++) //オーバーサンプリングをすると、音のアタックが薄れて音声が聞きづらいゲームがある（雀偵物語２,３等）ので、過剰なオーバーサンプリングしないほうがいいようだ。v2.62，v2.70
	{
		...
		if (_Phase >= (1<<13))
		{
			...
			{	//バイト後半のデータをデコード
				...
				{
					if (CDROM_GetHuVideo() && _bRepeat &&(_LengthCount >= 0x8000)&&(_LengthCount <= 0x80FF)) //ユナおまけCDで必要。1byte読む間に割り込みが起きないといけないのでディスクアクセスが速くない環境だと止まってしまう。_LengthCountの最上位ビットが立っていることは必須条件（でないとマンホールでフリーズ）。v2.83
					{	//ユナおまけCD,ガリバーボーイ。HuVideo以外でやると不具合。コブラ２のバイク発進シーンで固まる。v2.84更新
						_PlayedSampleCount--; //足踏みして待つ。実機にはできない処理だがタイミングが多少合わなくてもHuVideoが止まってしまわないための実装。
						bWait = TRUE;
					}
					else
					{	//通常
				...
				if (!bWait)
				{
					...
					//PRINTF("Kekka=%d, %d, %X, %X, %X, %X, %X",_bRepeat, _bStream, _PlayAddr, _PlayHalfAddr, _PlayedSampleCount, _PlayLength, _LengthCount);//test用
					if (_bRepeat &&(_LengthCount >= 0x8000)&&(_LengthCount <= 0x80FF)) //ガリバーボーイ、ユナおまけCD、聖戦士伝承(LengthCount=0x8000)で必要。v2.84
					{
						if (_PlayAddr == _PlayHalfAddr)
						{
							_PlayHalfAddr += 0x85; //0x85で途切れなく再生行ける。ガリバーボーイで133byte再生毎に割り込みを起こす。v2.83追加。ひろさんに掲示板で頂いた情報が大きなヒントになりました。ありがとうございます！
							if (_pfnNotification != NULL)
								_pfnNotification(ADPCM_STATE_HALF_PLAYED);
							//PRINTF("ADPCM HALF : %X,%X,%X,%X,%X,%X", _PlayAddr,_LengthCount,_ReadAddr,_WriteAddr,_PlayedSampleCount,_PlayLength); //test
						}							
					}
					else if (_LengthCount < 0x7FFF) //Kitao追加。(_LengthCount+1)が0x8000未満の場合、ハーフから_LengthCount毎の地点を分岐点とする。「トップを狙えVol.1」で必要。v1.07。
					{
...
//Kitao追加。_LengthCountを返す。v2.83
Uint32
ADPCM_GetLengthCount()
{
	return _LengthCount;
}

//Kitao追加。_bPlayを返す。v2.83
BOOL
ADPCM_GetPlay()
{
	return _bPlay;
