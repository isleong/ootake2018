...
recent_main()
{
	...
	while (_SelectedRom == 0)
	{
		...
		ok = (GetForegroundWindow() == _hWnd); //ウィンドウがアクティブならTRUE

		if ((ok)||(APP_GetOpenRomButton() >= 300)) //ROMファイルを開く。v2.86
		...
		if ((ok)||(APP_GetOpenCueButton() >= 300)) //CDインストールされたゲーム（CUE）を開く。v2.86
