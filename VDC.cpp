...
static BOOL		_bSpriteLayerR; //v2.90追加
static BOOL		_bBGLayerR; //v2.90追加
static BOOL		_bMaskedScreen; //v2.90追加
	...
	switch (regNum)
	{
		case 3: // 上位バイトの書き込み
			...
			switch (_AR[_VN])
			{
				...
				case VDC_LENR:
					//PRINTF("VRAM DMA Test!"); //test用
					do_vram_vram_dma();
					return;

				case VDC_SATB: //SATBのアドレスを設定＆転送の予約を入れる。
					//PRINTF("SATB Test!"); //test用
	...
	Sint32		drawFrame)
{
	...
	for (_VN=0; _VN<=_SuperGrafx; _VN++) //スーパーグラフィックスモードならVDC２つぶんを動かす
	{
		...
		if (_SpDmaCount[_VN] > 0)
		{
			...
			{
			//完了の割り込み要求
				if (DCR & 0x01) //DCR の D1 がセットされているときは完了割り込みを要求する
				{
					_VdcStatus[_VN] |= VDC_STAT_DS; //Kitao追記。(DCR & 0x01)が真のときだけステータスをセットする。"ドロップロックほらホラ"で必要。
					INTCTRL_Request(INTCTRL_IRQ1);
				}
			}
				...
				if (_ScreenH[_VN] >= 240) //240未満のとき調整すると、ドラキュラX最終面,プリンスオブペルシャで乱れ。マジカルチェイスのタイトル画面デモがループする時に音プチノイズが出る。フラッシュハイダース,カードエンジェルスで最下辺乱れ。v1.67
				{
					switch (MWR & 0x70) //マップサイズごとに分けないとダブルドラゴン２(MWR=0x70)で乱れ。
					{
						...
						case 0x40: //マップサイズが32x64の場合。
							...
							if ((DCR & 0x13) == 0x10) //VRAM->SATBへ自動転送処理を行う場合で、転送終了時の割り込みがオフの場合に遅めにVBlankを起こす。転送終了時オンのときに補正すると暗黒伝説のスタートデモ(DCR=0x13)で１フレーム暗転の乱れ。v2.80
								if (VCR >= 4) //VCRが標準以上。
						...
						case 0x00: //マップサイズが32x32の場合。
							if (DCR & 0x01) //VRAM->SATBへの転送終了後の割り込み処理を行う場合、遅めにVBlankを起こす。
							{
								...
								if (VCR < 4) //VCRが標準より小さい場合
									_VblankLineTop[_VN]++; //+1でロードランナー(パックインビデオ版)のEDIT時にカーソルの動きがよくなる。v2.81
							}
							if ((DCR & 0x13) == 0x10) //VRAM->SATBへ自動転送処理を行う場合で、転送終了時の割り込みがオフの場合に早めにVBlankを起こす。v2.80
							{
								if (VCR >= 4) //VCRが標準以上の場合。これがないとクロスワイバーのOPデモループ(VCR=3,MWR=0x10)でタイミング合わず。
									_VblankLineTop[_VN]--; //-1。-1であすか120%(MWR=0x10)の試合前デモの１フレーム乱れが解消。-1でパラソルスターの面クリア時(特に4-1や3-3面)の画面揺れが解消。-2以上引くとファージアスの邪皇帝戦闘シーン(MWR=0x10,VCR=0xF6)で乱れ。v2.50
								if (CR & 0x02) //スプライト表示異常割り込みを行う場合。v2.81追加
									_VblankLineTop[_VN] -= 2; //-2。-2でパラソルスターの面クリア時(特に4-1や3-3面)の画面揺れが解消。v2.81
							}
							break;
					}
				}
		...
		{
			//PRINTF("RCRtest = %d, %X %X %X %X",RCR,_VdcStatus[_VN],DCR,CR,_bUpdateSATB[_VN]); //Kitao Test用
			//PRINTF("RCRtest=%X, %d, %d, %d  [ %X , %X , %X , %X, %X , %X ] VBlineTop=%d",_DCC,_TvStartLine[_VN],DISPLAY_STARTLINE,_ScreenH[_VN], VPR,VDW,VCR,MWR,DCR,CR, _VblankLineTop[_VN]); //Kitao Test用
			if (CR & VDC_CTRL_RC)
				if ((VCR >= 4)||(VDW < 240)||((DCR & 0x01) == 0)||((CR & 0x40) == 0)||(RCR > 96)||((CR & 0x08) == 0)||(_bUpdateSATB[_VN])) //VCRが小さいときに、VRAM->SATBの割り込みオン時は上辺部でラスタ割り込みを起こさないようにした。ロードランナー(パックインビデオ版)のEDITモード。VDWが小さいときは起こす（テラフォーミングで必要）。v2.88更新
				{
	...
	{	//v1.41追加。割込み処理などを行うかどうかによって、ラスタ割り込みが発生するタイミングが変わる。
		...
		else if (_Regs[0][VDC_DCR] & 0x01) //VRAM->SATB間の転送終了時の割り込みを行う(DCR&0x1)場合、特殊とする。
		{
			if ((_Regs[0][VDC_MWR] & 0x60) == 0x00) //32x32か64x32キャラのBGマップサイズの場合
			{
				...
				else if (_Regs[0][VDC_VCR] < 4) //VCRの値が小さい場合
					rasterTimingCount = 35; //35。ロードランナー(パックインビデオ版)のEDITモードで小さめな値が最適。v2.81
				else
			...
			{	//VRAM転送関連の割込み処理などを行うとき
				switch (_Regs[0][VDC_MWR] & 0x70) //おそらくマップサイズごとにタイミングが異なる
				{
					...
					case 0x00: //32x32キャラのBGマップサイズ。
						rasterTimingCount = 66; //66。パワードリフトのDEMOで66以上なら乱れない。オペレーションウルフのスタートデモ(パラシュート落下&フェードアウト時)で66以下が必要。がんばれゴルフボーイズのタイトル画面で65辺りが最適（小さすぎると中央の雲が乱れ。小さくても大きくても乱れる)。ダウンタウン熱血行進曲大運動会で66以下が最適。v2.91更新
						break;
	...
	if ((_ScanLine == _VblankLineTop[0])||(_ScanLine == _VblankLineTop[_SuperGrafx])) //VBlankラインに来た
	{
		...
		//  プライベートアイドルのスタート直後のデモ(MWR=0x10。38.4NG。39.0NG。46.2OK。小さめの値が必要。小さすぎても大きすぎても駄目。VblankLineTopの値によって変化する)。
		...
		//  パワースポーツのニュース直前の9TV画面で揺れないために大きめの値が必要（MWR=0x10。SATB時のCPUストールの具合によって変化する。30.0OK。38.0,40.0OK）。
		...
		//	ソルジャーブレイドのOPデモ(MWR=0x10。66.0/100ぐらい以下が必要。36.0辺りが最適？。SATBの自動転送は無し)。
		//  メタルエンジェルのスタート後自己紹介後に止まらないために(MWR=0x40。36.0/100OK。小さいとマネージャー紹介後のフェードアウトがNG。大き目のほうがいいみたい。rasterTimingCountの値によって最適値が変わる)。
		//	大きい値だと、パラソルスターでステージ(1-9面,4-1面,3-3面で多い？)クリア時に稀に画面が揺れる(MWR=0x10。25.0でもタイミングによってはごく稀に揺れあり。実機でもたぶんありそうなのでこだわらず。34.0NG 61.2NG。VCR=3)。
		...
		//  レインボーアイランド(24.0NG。HURRY UP表示時に水色フラッシュの乱れ。27.2/100以上が必要)。
		...
		if (_Regs[0][VDC_MWR] & 0x40) //BGの縦マップサイズが64(MWRが0x40以上)の場合。v2.61
			...
			a = (int)(VDC_CYCLESPERLINE*30.0/100) - rasterTimingCount; // 30.0/100。レインボーアイランドOK。パワースポーツOK。パラソルスター4-1面,3-3面クリア時OK。プライベートアイドルOK。v2.81更新
																	   //           あすか120%OK。ワールドヒーローズ２。聖夜物語。ソルジャーブレイド。
		if (a < 0)
			a = 0;
		else
			cpuAdvance(a);
...
//Kitao追加。ゲームのタイミング不具合を解消するために、自動でオーバークロック設定にする。※ぽっぷるメイルでのみ使用。
void
...
VDC_SetLayer()
{
	...
	_bSpriteLayerR = _bSpriteLayer; //v2.90
	_bBGLayerR = _bBGLayer; //v2.90
	_bMaskedScreen = FALSE; //v2.90
}
	...
	BOOL  bMaskScreen)
{
	if (bMaskScreen)
	{
		_bSpriteLayerR = _bSpriteLayer;
		_bBGLayerR = _bBGLayer;
		_bSpriteLayer = FALSE;
		_bBGLayer = FALSE;
		_bMaskedScreen = TRUE;
	}
	...
	{
		_bSpriteLayer = _bSpriteLayerR;
		_bBGLayer = _bBGLayerR;
		_bMaskedScreen = FALSE;
	}
}
	...
	FILE*		p)
{
	...
	if (_bMaskedScreen) //特別に画面暗転処理の最中だった場合元に戻す。v2.90
	{
		_bSpriteLayer = _bSpriteLayerR;
		_bBGLayer = _bBGLayerR;
		_bMaskedScreen = FALSE;
