...
・端子READ時にbit4とbit5は真を返す。マジクールでマルチタップを認識しなかた問題
  が解消。v2.80
...
static BOOL		_bChange3to12; //Kitao追加。v2.87
...
read_two_button_pad()
{
	...
	Uint8	b1, b2;
	Uint32	padNumber;
	BOOL	bIIIrButtonPushed = FALSE; //Kitao追加
	BOOL	bIIIsButtonPushed = FALSE; //Kitao追加
	BOOL	bR; //v2.87追加
	Sint16	rRenshaSpeed1, rRenshaSpeed2, rRenshaSpeedRun; //v2.85

	if (_Counter == 0)
		return 0x30; //v1.10更新。エメラルドドラゴンでMB128ユーティリティが動作。ガンヘッドの面セレクト裏技もOK（裏技入力時に連射ボタンで設定しないように注意）。Ｆ１サーカスの車スタートもOK。

	if (_bMultiTap)
	{
		if (_Counter > 5)
			return 0x30; //v2.80更新。0x30でマジクールでマルチタップ認識OKに。ここで0x00を返すと真怨霊戦記で入力不具合。
		padNumber = _Counter;
	}
	else
	...
	{
		if (INPUT_GetGameSetting()) //自動押しっぱなし機能を使っている場合、連射機能を一時的にオフにする。
		{
			rRenshaSpeed1   = _RenshaSpeed[1].button1;
			rRenshaSpeed2   = _RenshaSpeed[1].button2;
			rRenshaSpeedRun = _RenshaSpeed[1].buttonRun;
		...
		if (INPUT_IsPressed(padNumber, JOYPAD_BUTTON_SELECT))
			ret &= ~0x4;
		if (INPUT_IsPressed(padNumber, JOYPAD_BUTTON_IV)) //３ボタンパッド設定の時用。IIIsボタン
		{
			ret &= ~0x4;
			bIIIsButtonPushed = TRUE;
		}

		if (INPUT_IsPressed(padNumber, JOYPAD_BUTTON_RUN))
			ret &= ~0x8;
		if (INPUT_IsPressed(padNumber, JOYPAD_BUTTON_III)) //３ボタンパッド設定の時用。IIIrボタン
		{
			ret &= ~0x8;
			bIIIrButtonPushed = TRUE;
		}

		//ボタン入れ換え設定の処理
		if (_bSwapSelRun) //_bChange3to12の判定より先に行う必要がある
			if (!_bThreeButtonPad || bIIIrButtonPushed || bIIIsButtonPushed) //３ボタンパッドの場合、IIIrかIIIsボタンが押されたときのみ入れ替えを行う(通常のSELECT,RUNボタンが押されたときは入れ換えない)。
			{	//RUNボタンとSELECTボタンを入れ替え
				b1 = (ret & 0x8) >> 1;
				b2 = (ret & 0x4) << 1;
				ret &= ~0xC;
				ret |= b1 | b2;
				bR = bIIIrButtonPushed;
				bIIIrButtonPushed = bIIIsButtonPushed;
				bIIIsButtonPushed = bR;
			}
		if (_bChange3to12)
			if (_bThreeButtonPad) //３ボタンパッド時のみ
			{	//IIIrボタンをI+IIに置き換え。IIIsボタンをIIIrボタンに置き換え。v2.87
				if (bIIIrButtonPushed) //IIIrボタンが押された（またはSwap SEL&RUN設定でIIIsボタンが押された）
				{
					if (!INPUT_IsPressed(padNumber, JOYPAD_BUTTON_RUN)) //RUNを押していないときだけ
						ret |= 0x8; //RUNボタンを離す
					ret &= ~0x3; //IボタンとIIボタンを押す
				}
				if (bIIIsButtonPushed) //IIIsボタンが押された（またはSwap SEL&RUN設定でIIIrボタンが押された）
				{
					if (!INPUT_IsPressed(padNumber, JOYPAD_BUTTON_SELECT)) //SELECTを押していないときだけ
						ret |= 0x4; //SELECTボタンを離す
					ret &= ~0x8; //RUNボタンを押す
				}
			}
		if (_bSwapIandII)
		...
		if (INPUT_GetGameSetting()) //自動押しっぱなし機能を使っている場合、連射機能設定を元に戻す。
		{
			_RenshaSpeed[1].button1   = rRenshaSpeed1;
			_RenshaSpeed[1].button2   = rRenshaSpeed2;
			_RenshaSpeed[1].buttonRun = rRenshaSpeedRun;
		}
	}
	...
	if ((padNumber == 1)&&(INPUT_CheckButtonState(APP_GetFunctionButton())))
		if (APP_CheckShortcutKey(APP_GetFunctionButton())) //ファンクションボタンがPCEのボタンと被っていた場合(FALSEが返る)は、ファンクションボタンが無効なので入力を受け付ける。v2.77
			return 0x3F; //入力を受け付けない

	return ret | 0x30; //v2.80更新
}
...
read_six_button_pad()
{
	...
	if (_Counter == 0)
		return 0x30; //v1.10更新。エメラルドドラゴンでMB128ユーティリティが動作。ガンヘッドの面セレクト裏技もOK。
...
//Kitao追加。v2.87
void
JOYPAD_SetChange3to12(
	BOOL	bChange3to12)
{
	_bChange3to12 = bChange3to12;
}
...
//Kitao追加。App.cから使用。v2.87
BOOL
JOYPAD_GetChange3to12()
{
	return _bChange3to12;
}
...
*/
	...
	//前回アップデートしてから16ライン描画以上の時間が立っていたらアップデート。アップデートのし過ぎで重くなることを回避。
	//v2.90更新。16ライン毎(1フレームで約16回。1000Hz弱)で、約1000Hzのゲームパッド使用時に入力反応が良くなる意味がある（エミュレーション処理を4分割するHigh Quality PSG設定(default)時）。
	//           ゲーミング用のUSBマウス等で1000Hz動作なのでこれに合わせた。PS4のパッドをPCにつなげた時に250Hz。これ以上処理するとパワーの無いPCで動作が重いため16ライン毎とした。
 	if (MAINBOARD_GetScanLine()/16 != _PrevUpdateStateLine/16)
	{
	...
	FILE*		p)
{
	...
	if (MAINBOARD_GetStateVersion() >= 49) //v2.87以降のセーブファイルなら
	{
		LOAD_V(_bChange3to12); //v2.87追加
	}
	else
		_bChange3to12 = FALSE;
