...
static Sint32		_DdaFadeOutL[8]; //Kitao追加
static Sint32		_DdaFadeOutR[8]; //Kitao追加
static Uint32		_Channel;				// 0 - 5;
static Uint32		_MainVolumeL;			// 0 - 15
static Uint32		_MainVolumeR;			// 0 - 15
static Uint32		_LfoFrq;
static BOOL			_bLfoOn; //v1.59から非使用。過去verのステートロードのために残してある。
static Uint32		_LfoCtrl;
static Uint32		_LfoShift; //v1.59から非使用。過去verのステートロードのために残してある。
static Sint32		_PsgVolumeEffect = 0;//Kitao追加
static double		_Volume = 0;//Kitao追加
static double		_VOL = 0.0;//Kitao追加。v1.08
static BOOL			_bPsgMute[8] = {FALSE,FALSE,FALSE,FALSE,FALSE,FALSE,FALSE,FALSE};//Kitao追加。v1.29
static BOOL			_bWaveReset[8]; //v2.81追加
...
static BOOL			_bHoneyInTheSky; //はにいいんざすかいパッチ用。v2.60
	...
	Uint8		data)
{
	...
	switch (reg & 15)
	{
		...
		case 1:	// main volume
			...
			{
				if (_Psg[i].volumeL == 0) //volumeLが0の場合、volumeの値があってもL音量は0とする。スーパーバレーボールの笛後やイメージファイトのやられた後等で必要。※volumeだけが0のときにL音量を0にしてしまうとグラディウスのコナミ起動音で不具合。v2.81追加
					_Psg[i].outVolumeL = 0;
				else
					_Psg[i].outVolumeL = _VolumeTable[_Psg[i].volume + (_MainVolumeL + _Psg[i].volumeL) * 2];
				if (_Psg[i].volumeR == 0) //volumeRが0の場合、volumeの値があってもR音量は0とする。スーパーバレーボールの笛後やイメージファイトのやられた後等で必要。v2.81追加
					_Psg[i].outVolumeR = 0;
				else
		...
		case 2:	// frequency low
			...
			if (frq)
				_Psg[_Channel].deltaPhase = (Uint32)((double)(65536.0 * 256.0 * 8.0 * PSG_FRQ / OVERSAMPLE_RATE / SAMPLE_RATE) / (double)frq +0.5); //Kitao更新。速度アップのためfrq以外は定数計算にした。精度向上のため、先に値の小さいOVERSAMPLE_RATEのほうで割るようにした。+0.5は四捨五入で精度アップ。プチノイズ軽減のため。
			else
		...
		case 4:	// ON, DDA, AL
			if (_bHoneyInTheSky) //はにいいんざすかいのポーズ時に、微妙なボリューム調整タイミングの問題でプチノイズが載ってしまうので、現状はパッチ処理で対応。v2.60更新
			{
				if ((_Psg[_Channel].bOn)&&(data == 0)) //発声中にdataが0の場合、LRボリュームも0にリセット。はにいいんざすかいのポーズ時のノイズが解消。(data & 0x1F)だけが0のときにリセットすると、サイレントデバッガーズ等でNG。発声してない時にリセットするとアトミックロボでNG。ｖ2.55
				{
					//PRINTF("test %X %X %X %X",_Channel,_Psg[_Channel].bOn,_MainVolumeL,_MainVolumeR);
					if ((_MainVolumeL & 1) == 0) //メインボリュームのbit0が0のときだけ処理(はにいいんざすかいでイレギュラーな0xE。他のゲームは0xF。※ヘビーユニットも0xEだった)。これがないとミズバク大冒険で音が出ない。実機の仕組みと同じかどうかは未確認。v2.53追加
						_Psg[_Channel].volumeL = 0;
					if ((_MainVolumeR & 1) == 0) //右チャンネルも同様とする
			...
			if ((_Psg[_Channel].bDDA)&&((data & 0x40)==0)) //DDAからWAVEへ切り替わるとき or DDAから消音するとき
			{
			...
			if ((data & 0xC0) == 0x40)
			{
				if ((_Psg[_Channel].bDDA)&&(_Psg[_Channel].waveIndex == 0)&& //DDAモードでwaveIndexが0のとき、Waveデータをリセットする。実機で同様の動きかは未確認。F1トリプルバトル。フォーメーションサッカー'90試合中BGM。v2.79
					((_Psg[_Channel].volumeL > 0)||(_Psg[_Channel].volumeR > 0))) //Volumeが0のときに行うとゼビウスでPAUSE後音が薄くなる。v2.82追加
				{
					//PRINTF("test4 %X %X %X %X %X",_Channel,data,_Psg[_Channel].waveIndex,_Psg[_Channel].volumeL,_Psg[_Channel].volumeR); //Kitaoテスト用
					_bWaveReset[_Channel] = TRUE; //v2.81追加。フォーメーションサッカー'90，F1トリプルバトル，ファイプロシリーズで必要。
				}
				else
					_bWaveReset[_Channel] = FALSE; //v2.81追加
				_Psg[_Channel].waveIndex = 0;
			}

			_Psg[_Channel].volume = data & 0x1F;
			if ((_Psg[_Channel].volume == 0)||(_Psg[_Channel].volumeL == 0)) //スーパーバレーボールの笛後等に必要。v2.81追加
				_Psg[_Channel].outVolumeL = 0;
			else
				_Psg[_Channel].outVolumeL = _VolumeTable[_Psg[_Channel].volume + (_MainVolumeL + _Psg[_Channel].volumeL) * 2];
			if ((_Psg[_Channel].volume == 0)||(_Psg[_Channel].volumeR == 0)) //スーパーバレーボールの笛後等に必要。v2.81追加
				_Psg[_Channel].outVolumeR = 0;
			else
				_Psg[_Channel].outVolumeR = _VolumeTable[_Psg[_Channel].volume + (_MainVolumeR + _Psg[_Channel].volumeR) * 2];
			break;

		case 5:	// LAL, RAL
			//PRINTF("test5 %X %X %X %X %X",_Channel,data,_Psg[_Channel].waveIndex,_Psg[_Channel].volumeL,_Psg[_Channel].volumeR); //Kitaoテスト用
			_Psg[_Channel].volumeL = (data >> 4) & 0xF;
			_Psg[_Channel].volumeR = data & 0xF;
			if ((_Psg[_Channel].volume == 0)||(_Psg[_Channel].volumeL == 0)) //グラディウスでポーズ時に必要。v2.81追加
				_Psg[_Channel].outVolumeL = 0;
			else
				_Psg[_Channel].outVolumeL = _VolumeTable[_Psg[_Channel].volume + (_MainVolumeL + _Psg[_Channel].volumeL) * 2];
			if ((_Psg[_Channel].volume == 0)||(_Psg[_Channel].volumeR == 0)) //グラディウスでポーズ時に必要。v2.81追加
				_Psg[_Channel].outVolumeR = 0;
			else
				_Psg[_Channel].outVolumeR = _VolumeTable[_Psg[_Channel].volume + (_MainVolumeR + _Psg[_Channel].volumeR) * 2];
			break;
		...
		case 6:	// wave data  //Kitao更新。DDAモードのときもWaveデータを更新するようにした。v0.63。ファイヤープロレスリング
			...
			if ((_bWaveReset[_Channel])&&(_Psg[_Channel].bDDA)&&(_Psg[_Channel].waveIndex == 0)&& //DDAモードでwaveIndexが0のとき、Waveデータをリセットする。実機で同様の動きかは未確認。F1トリプルバトル。フォーメーションサッカー'90試合中BGM。v2.79
				((_Psg[_Channel].volumeL > 0)||(_Psg[_Channel].volumeR > 0))) //Volumeが0のときに行うとゼビウスでPAUSE後音が薄くなる。v2.82追加
			{
				//PRINTF("test6 %X %X %X %X %X",_Channel,data,_Psg[_Channel].waveIndex,_Psg[_Channel].volumeL,_Psg[_Channel].volumeR); //Kitaoテスト用
				for (i=0; i<32; i++)
					_Psg[_Channel].wave[i] = -14; //最小値で初期化。フォーメーションサッカー'90，F1トリプルバトル，ファイプロシリーズで必要。
			}
			else
				_bWaveReset[_Channel] = FALSE; //v2.82追加

			if (!_Psg[_Channel].bOn) //Kitao追加。音を鳴らしていないときだけWaveデータを更新する。v0.65。F1トリプルバトルのエンジン音。
			{
				_Psg[_Channel].wave[_Psg[_Channel].waveIndex++] = 17 - data; //17。Kitao更新。一番心地よく響く値に。ミズバク大冒険，モトローダー，ドラゴンスピリット等で調整。
			...
			if (_Psg[_Channel].bDDA)
			{
				...
				_Psg[_Channel].ddaSample = 11 - data; //サイバーナイトで11に決定。ドラムの音色が最適。v0.74
			}
			break;
		...
		case 7:	// noise on, noise frq
			if (_Channel >= 4)
			{
				...
				if (_Psg[_Channel].noiseFrq == 0)
					_Psg[_Channel].deltaNoisePhase = (Uint32)((double)(2048.0 * PSG_FRQ / OVERSAMPLE_RATE / SAMPLE_RATE) +0.5); //Kitao更新
				else
					_Psg[_Channel].deltaNoisePhase = (Uint32)((double)(2048.0 * PSG_FRQ / OVERSAMPLE_RATE / SAMPLE_RATE) / (double)_Psg[_Channel].noiseFrq +0.5); //Kitao更新
			}
			break;
		...
		case 9:	// LFO control  Kitao更新。シンプルに実装してみた。実機で同じ動作かは未確認。はにいいんざすかいの音が似るように実装。v1.59
			...
			{
				_Psg[1].phase = 0; //LfoFrqは初期化しない。はにいいんざすかい。
				//Kitaoテスト用
				//PRINTF("LFO control = %X",data);
			}
			_LfoCtrl = data & 7; //ドロップロックほらホラで5が使われる。v1.61更新
			if (_LfoCtrl & 4)
				_LfoCtrl = 0; //ドロップロックほらホラ。実機で聴いた感じはLFOオフと同じ音のようなのでbit2が立っていた(負の数扱い？)ら0と同じこととする。
			//Kitaoテスト用
...
set_VOL()
{
	...
	if (_PsgVolumeEffect == 0)
		...
		_VOL = _Volume / (double)(OVERSAMPLE_RATE * 4.0/3.0); // 3/4。v1.29追加
	else
		_VOL = _Volume / (double)(OVERSAMPLE_RATE * _PsgVolumeEffect); //Kitao追加。_PsgVolumeEffect=ボリューム調節効果。
	...
	Sint32		i)
{
	...
	if ((i==0)&&(_LfoCtrl>0))
	{
		...
		lfo = _Psg[1].wave[_Psg[1].phase >> 27] << ((_LfoCtrl-1) << 1); //v1.60更新
		_Psg[0].phase += (Uint32)((double)(65536.0 * 256.0 * 8.0 * PSG_FRQ / OVERSAMPLE_RATE / SAMPLE_RATE) / (double)(_Psg[0].frq + lfo) +0.5);
		_Psg[1].phase += (Uint32)((double)(65536.0 * 256.0 * 8.0 * PSG_FRQ / OVERSAMPLE_RATE / SAMPLE_RATE) / (double)(_Psg[1].frq * _LfoFrq) +0.5); //v1.60更新
	}
	else
	...
	Sint32		nSample)	// 書き出すサンプル数 
{
	...
	for (j=0; j<nSample; j++)
	{
		...
		for (i=0; i<N_CHANNEL; i++)
		{
			if ((_Psg[i].bOn)&&((i != 1)||(_LfoCtrl == 0))&&(!_bPsgMute[i])) //Kitao更新
			{
				if (_Psg[i].bDDA)
				{
					smp = _Psg[i].ddaSample * _Psg[i].outVolumeL;
					//smp = _Psg[i].ddaSample * _Psg[i].outVolumeL * (Sint32)OVERSAMPLE_RATE; //オーバーサンプリング用
					...
					//smp = _Psg[i].ddaSample * _Psg[i].outVolumeR * (Sint32)OVERSAMPLE_RATE; //オーバーサンプリング用
				...
				else if (_Psg[i].bNoiseOn)
				{
					...
					_Psg[i].phase += _Psg[i].deltaNoisePhase; //Kitao更新
					/*
					...
					*/
					...
					if (_Psg[i].noiseFrq == 0) //Kitao追加。noiseFrq=0(dataに0x1Fが書き込まれた)のときは音量が通常の半分とした。（ファイヤープロレスリング３、パックランド、桃太郎活劇、がんばれゴルフボーイズなど）
					{
				...
				else if (_Psg[i].deltaPhase)
				{
					...
					*/
					...
					sampleAllL += sample * _Psg[i].outVolumeL; //Kitao更新
					sampleAllR += sample * _Psg[i].outVolumeR; //Kitao更新
				}
			}
			...
			//sampleAllL += _DdaFadeOutL[i] * OVERSAMPLE_RATE; //オーバーサンプリング用
			sampleAllR += _DdaFadeOutR[i];
			//sampleAllR += _DdaFadeOutR[i] * OVERSAMPLE_RATE; //オーバーサンプリング用
		}
		//Kitao更新。6ch合わさったところで、ボリューム調整してバッファに書き込む。オーバーサンプリング(※現在非使用)したぶんも_VOLによってここで割られて調整される。
...
psg_reset()
{
	...
	memset(_DdaFadeOutL, 0, sizeof(_DdaFadeOutL)); //Kitao追加
	memset(_DdaFadeOutR, 0, sizeof(_DdaFadeOutR)); //Kitao追加
	_MainVolumeL = 0;
	_MainVolumeR = 0;
	_LfoFrq = 0;
	_LfoCtrl = 0;
	memset(_bWaveReset, 0, sizeof(_bWaveReset)); //Kitao追加。v2.81

	//Kitao更新。v0.65．waveデータを初期化。
	for (_Channel=0; _Channel<=5; _Channel++)
		for (i=0; i<32; i++)
			_Psg[_Channel].wave[i] = 17; //ch3は最大値で初期化。F1トリプルバトル。ファイプロ。v2.65
	_Channel = 0; //Kitao追加。v2.65
	...
	FILE*		p)
{
	...
	SAVE_A(_bWaveReset); //v2.81追加

	SAVE_V(bFlashHiders); //Kitao追加。v0.62。現在非使用。旧バージョンのステートセーブとの互換のため
	...
	FILE*		p)
{
	...
	BOOL			bWaveCrash; //Kitao更新。現在非使用。旧バージョンのステートセーブとの互換のため
	BOOL			bWaveCrash2[8]; //Kitao更新。現在非使用。旧バージョンのステートセーブとの互換のため
	...
	{
		if (MAINBOARD_GetStateVersion() == 46) //v2.79betaのセーブファイルなら
		{
			LOAD_A(bWaveCrash2); //現在非使用
		}
		else if (MAINBOARD_GetStateVersion() <= 45) //v2.78以前のセーブファイルなら
		{
			LOAD_V(bWaveCrash); //現在非使用
		}
	}
	if (MAINBOARD_GetStateVersion() >= 48 ) //v2.81以降のセーブファイルなら
	{
		LOAD_A(_bWaveReset);
	}
	else
	{
		memset(_bWaveReset, 0, sizeof(_bWaveReset));
	}
