...
static UINT		_MaxFrameLatency = 1; //ビデオのコントロールパネルで設定されている「最大レンダリングフレーム数」。XPでは得ることができないので1としておく。
...
static BOOL		_bIntelVideoChip = FALSE; //インテルの内蔵ビデオチップの場合TRUEに。v2.79
...
d3dCreate()
{
	...
	{	//v2.21追加
		...
		_pD3DDevEx->GetMaximumFrameLatency(&_MaxFrameLatency); //コントロールパネルで設定されているレンダリングフレーム数を得る。コントロールパネルで設定していても3が必ず返ってくるようなので現状使えず。v2.88
		_pD3DDevEx->SetMaximumFrameLatency(1); //遅延を最小限に抑える。Aeroオフ＋これでやっとXP並に。※コントロールパネルの設定によっては効かない（多くの環境ではデフォルト設定で効かない）ので気休め程度。v2.88更新
	...
	Uint32		flags)
{
	...
	{
		//PRINTF("Intel! VideoChip = %s ", di.Description); //Test用
		_bIntelVideoChip = TRUE; //インテルのIrisより前のローパワー内蔵ビデオチップ。
	}
	...
	if ((APP_GetSyncTo60HzScreen())&&
		...
		(APP_GetWindowsAutoVSyncBool())) //リフレッシュレートが60Hz(59と61も含む)か120Hz、またはWindows自動V-Sync設定のときだけVSyncをOKに。
	{
	...
	BOOL	bDraw) //bDrawをTRUEにして呼ぶと描画も行う。FALSEの場合VSync待ちのみ。
{
	...
	if (APP_GetWindowsAutoVSyncBool()) //v2.65追加。明示的にWindowsの自動V-Syncを使うモードを設置。表示遅延が起こるので非推奨。ブラウン管なら大丈夫かな。
	{
		//Waitする前に描画処理を済ませる。
		if (bDraw)
		{
			...
			UpdateWindow(_hWnd); //遅延対策。v2.89
		}
		return TRUE;
	}

	//Vista以降で"Aero"か"Vistaスタンダード(Home Basicにのみ存在)"，"Win8/10"のデスクトップ利用時は、Windows自体でV-Syncを自動で行うので、処理がぶつかって遅くならないようにOotake側はV-Syncに構わず、先に描画を済ませる。v2.02
	if ((APP_GetWindowsVista())&&(APP_DwmIsCompositionEnabled()))
	{
		...
		if (bDraw)
		{
			...
			UpdateWindow(_hWnd); //遅延対策。v2.89
			if (APP_GetRemoveDisplayLagFrame() >= 1) //表示ラグ削減モード（デフォルト）の場合。v2.89
			{
				hr = _pD3DDev->Present(NULL, NULL, NULL, NULL);
				UpdateWindow(_hWnd); //遅延対策。v2.89
				if (APP_GetRemoveDisplayLagFrame() >= 3) //表示ラグ削減モード3F（デフォルト）の場合
				{	//Aero(&Win8/10)の場合、レンダリングバッファがもう2フレームぶんあるのでそれも消化する
					hr = _pD3DDev->Present(NULL, NULL, NULL, NULL);
					UpdateWindow(_hWnd); //遅延対策。v2.89
					hr = _pD3DDev->Present(NULL, NULL, NULL, NULL);
					UpdateWindow(_hWnd); //遅延対策。v2.89
	...
	if (_bSyncTo60HzScreen)
	{
		...
		if (t <= vt) //17ms(1/60s=16.7ms)以上経過していた場合は待たない。(timeGetTime()の誤差があるのでt=17でもまだ16.1ms経過のこともあるが二度待ちしないことを優先）
		{
			//他のアプリのために処理を一休み。※VBlank待ちループ中はCPU占有率が100%になるので、これをやると省電力。＆Sleepを入れない場合、連続してWaitする場合に処理されないことがあるので、これが必要。
			if (vt == 16)
			{
				t = 16 - t - 4; //-4は処理落ちしないためのマージン。-4OK。引かな過ぎるとテンポの揺らぎからわずかに音が篭る気がする。16=１コマの時間約16ms。
								//-5だとCDDAタイミング測定時(IntelAtom640x480fullscreen環境で確認)にSleepが短すぎて連続のVSyncがカットされてしまうことがあった。
				if ((t >= 1)&&(t <= 16-4)) //タイマーカウンタのオーバーフローも考えて、この範囲のときだけSleepをおこなう。
					Sleep(t);
			}
			else //vt=8(for 120Hz)
			{
				t = 8 - t - 2; //-2は処理落ちしないためのマージン。-2OK。-4だとSleepが短すぎて連続のVSyncがカットされてしまうことがあった。8=１コマの時間約8ms。
				if ((t >= 1)&&(t <= 8-2))
					Sleep(t);
			}
		...
		if (bDraw)
		{
			...
			UpdateWindow(_hWnd); //遅延対策。v2.89
			//即時にレンダリングバッファを消化するために、同じ画面をもう一度描画処理する。遅延解消。v2.88追加
			if (APP_GetRemoveDisplayLagFrame() >= 1) //表示ラグ削減モード（デフォルト）の場合
			{
				hr = _pD3DDev->Present(NULL, NULL, NULL, NULL); //今表示したいものを最前面にするため、更に同じ内容でバッファを更新。1フレーム遅延削減効果あり
				UpdateWindow(_hWnd); //遅延対策。v2.89
				//if (APP_GetRemoveDisplayLagFrame() >= 3) //表示ラグ削減モード3F（デフォルト）の場合
				//{
				//	hr = _pD3DDev->Present(NULL, NULL, NULL, NULL); //Aero/8/10のみ
				//	UpdateWindow(_hWnd); //遅延対策。v2.89
				//	hr = _pD3DDev->Present(NULL, NULL, NULL, NULL); //Aero/8/10のみ
				//	UpdateWindow(_hWnd); //遅延対策。v2.89
				//}
	...
	Sint32		executeCode)
{
	...
	if (IsIconic(_hWnd)) //ウィンドウが最小化されている時。hesを最小化して聴くときなどに処理を軽くする。
		...
		{
			//垂直帰線期間を待つ
			SCREEN_WaitVBlank(FALSE); //描画処理は行わない
			return;
		}
	...
	BOOL	bSyncTo60HzScreen)
{
	...
	SCREEN_SetVSync120Hz(FALSE);
	//SCREEN_SetVSync120Hz(TRUE);//test用
	memset(&dm, 0, sizeof(DEVMODE));
	dm.dmSize = sizeof(DEVMODE);
	EnumDisplaySettings(NULL, ENUM_CURRENT_SETTINGS, &dm);
	if ((dm.dmDisplayFrequency == 60)||(dm.dmDisplayFrequency == 59)||(dm.dmDisplayFrequency == 61)||   //リフレッシュレートが60Hz(59と61も含む)か強制設定のとき。ForceVSyncはv2.65から廃止。
		(dm.dmDisplayFrequency == 120)||(dm.dmDisplayFrequency == 119)||(dm.dmDisplayFrequency == 121)) //リフレッシュレートが120Hz(119と121も含む)のとき
	{
		_bSyncTo60HzScreen = APP_GetSyncTo60HzScreen();
		APP_EnableVSyncMenu(TRUE); //メニュー更新
		if ((dm.dmDisplayFrequency == 120)||(dm.dmDisplayFrequency == 119)||(dm.dmDisplayFrequency == 121)) //リフレッシュレートが120Hz(119と121も含む)のとき
			SCREEN_SetVSync120Hz(TRUE);
	}
	else
	{
