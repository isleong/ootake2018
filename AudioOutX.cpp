...
HMODULE g_XAudioDLL	= NULL; //v2.85追加
...
x_deinit() 	//Kitao更新。3chぶんのリソースを開放
{
	...
	//Win8以降でIMEが英語モード以外になっているとappCrashを引き起こす可能性があるのでその対処。v2.85
	if (g_XAudioDLL)
	{
	...
	DWORD	dwBufSize)			// in bytes
{
	...
	//Win8以降でIMEが英語モード以外になっているとappCrashを引き起こす可能性があるのでその対処。v2.85
	g_XAudioDLL = LoadLibraryEx("XAudio2_7.DLL", NULL, 0x00000800/* LOAD_LIBRARY_SEARCH_SYSTEM32 */ ); //v2.86更新。DirectX SDKを新しくしたのでこのバージョンをロード。
	if (g_XAudioDLL == NULL)
		g_XAudioDLL = LoadLibraryEx("XAudio2_6.DLL", NULL, 0x00000800/* LOAD_LIBRARY_SEARCH_SYSTEM32 */ );
